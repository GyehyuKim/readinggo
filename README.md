...
"읽고 싶은데 이어가지 못하는 사람"을 위한 독서 습관 앱. v18 목표:

- **마찰 최소** — 목표 페이지를 강제하지 않고 하루 한 페이지·한 문장부터 다시 책을 펼친다.
- **한 흐름의 서재** — 읽고 싶은 책·읽는 중·읽은 책·중단한 책을 하나의 연속 흐름에서 탐색하고 상태 포함·제외와 정렬을 적용한다.
...
- **공개 안전** — 친구 책나무 표면은 보류하지만 허용된 공개 문장만 서버에서 판정하고 private 문장의 존재·개수·상호작용을 숨기는 계약은 유지한다.
- **폐기 대상** — XP·둥지 진화·스트릭 상실·방패·성 컬렉션을 신규 제품 보상으로 사용하지 않는다.
- **보류 대상** — 책=가지·문장=잎의 한 그루 책나무 시각화는 이력·연구·코드를 보존하되 활성 내비게이션과 신규 구현에서 제외한다.
...
- **현행 as-built** — 웹·모바일 데모에는 3번째 `책나무`(`nest-grow`), 4번째 `프로필`과 XP·스트릭·둥지·broad 공개 경로가 남아 있다. 이는 v18 완료가 아니라 전환·삭제 대상이다.
...
| 이름 | 역할 | 작업 방식 |
|---|---|---|
| **계휴** (`gyehyu`) | maintainer · Production 승격 | 제품·아키텍처·통합, 검증된 DEV SHA의 Production 승격 |
| **승원** (`seungwon`) | contributor | 사전 합의된 이슈 범위의 제안·검토·구현·PR·main/DEV merge |
| **윤지** (`jyj23-jeong`, `yunji`) | contributor | 사전 합의된 이슈 범위의 제안·검토·구현·PR·main/DEV merge |

- 파일·기능별 고정 담당은 두지 않는다.
- 브랜치: `gyehyu/<topic-slug>` · `seungwon/<topic-slug>` · `yunji/<topic-slug>`. **spec PR과 코드 PR은 분리.** 감독 게이트를 통과한 PR은 계휴·Hermes·승인 contributor가 `main`에 merge하고 stable DEV까지 검증할 수 있다. Production 승격은 계휴만 수행한다.
