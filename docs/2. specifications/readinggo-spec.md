...
> **v6 주요 변경** (2026-05-28 피드백 × 스펙 교차 분석 반영):
> - 마을 탭 → **독서모임 탭** 전면 대체 (§5.5)
> - **AI 도서 추천** 신설 — 완독 후 다음 책 3권 (§5.8, Phase 2)
> - **AI 추출 책** 신설 — 완독 모이를 AI가 재구성 (§5.8, Phase 2)
> - **SNS 이미지 카드** 세부 스펙 확정 (§5.10, Phase 1 P1)
> - **마일스톤 공유 카드** 신설 — 50·100일 스트릭 (§6.3)
> - T2 mini(§13.2) + 마을 탭 재설계(§13.4) → 독서모임으로 해소
...
| **Phase 2** | 학기말 발표 — 앱스토어 출시 | iOS App Store + Google Play. OCR · 위젯 · 자동 백업 · **AI 도서 추천** · **AI 추출 책** | Supabase + FCM + APNs + Gemini API | Postgres + 로컬 캐시 |
| **Phase 3** | 학기 후 지속 개발 | 컬렉션·자유 노트 별도 타입(검토)·음성 받아쓰기·챕터 자동 인식·수익 모델 검토·**독서모임 완독 레이스 실시간** | 동일 | 동일 |
...
| ~~마을 탭 설계~~ | **v6 해소** — 독서모임 탭으로 전면 대체 (§5.5) |
...
| ~~**v5 — T2 mini**~~ | **v6 해소** — 독서모임 탭 메가 스트림으로 흡수 (§5.5) |
| ~~**v5 — T2 좀비 사용자 처리**~~ | **v6 해소** — 독서모임 가입자 전용 공개 원칙으로 대체 (§5.5) |
...
| **v5.1 — 수익 모델 검토** | 학기 후 별도 안건 (§13.6). 어필리에이트(교보문고)만 Phase 1 파라미터 추가로 즉시 적용 |
| **v5.1 — Rich Text 자유 노트** | *별도 타입 신설은 보류*. 모이 확장 필드(`my_note`)로 흡수. 별도 타입 채택 여부는 학기 후 (§13.5) |
| **v6 — 독서모임 탭** | 마을 탭 대체. 메가 스트림(자동) + 서브 모임(공개/비공개). 구경 불가. (§5.5) |
...
| **v6 — SNS 이미지 카드 세부 스펙** | §5.10 신설. 모이 카드 + 마일스톤 카드. Phase 1 P1 |
| **v6 — 마일스톤 공유 카드** | 50일·100일 스트릭 달성 시 특별 confetti + 공유 카드 자동 생성. (§6.3) |
...
> **v6 해소** — 독서모임 탭 §5.5 의 메가 스트림 구조로 흡수·확장. 별도 안건 종료.
>
...
> **v6 해소** — 독서모임 탭(§5.5)으로 전면 대체 결정. 별도 안건 종료.
>
