...
-- `schema.sql` is applied before ordered migrations. The canonical public gate
-- arrives in 65_book_privacy_foundation.sql, so defer only dependency binding;
-- the RPC still fails closed until that migration has installed the function.
