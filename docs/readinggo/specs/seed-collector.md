...
1. **검색**: `https://www.yes24.com/Product/Search?domain=ALL&query={제목 저자}` 를 **브라우저로** 열고 `networkidle` 대기(JS 렌더).
2. **상품 매칭**: 검색 결과에서 책 링크 추출 — 패턴 **소문자** `/product/goods/{id}`. 첫 결과 우선, **ISBN13 대조**(상품페이지 ISBN 일치 확인)로 정확 매칭. 동명이서·다른 판본 방지.
3. **발췌 추출**: 상품페이지 `innerText`에서 **"책 속으로"** 섹션 → `* ` 로 구분된 발췌문 파싱.
...
> **검증된 셀렉터·패턴 (2026-06-18)**: 제목+저자 검색은 렌더됨(ISBN 단독 검색은 결과 비어 권장 안 함). 책 링크는 `a[href^="/product/goods/"]`. "책 속으로"는 `body.innerText.indexOf('책 속으로')` 이후 `* ` 분할. 모순 6개 발췌 확인.
