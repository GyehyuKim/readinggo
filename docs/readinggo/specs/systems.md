...
> **v5 위상 변경**: 첫 7일 보호의 *주축* 은 [§5.6](./book-club.md) 운영자 짹으로 이전. 방패는 *보조 안전망* 으로 위상 조정. 메카닉 자체는 v4.4 유지.
...
**XP destination — 미해결 ([§13](./meta/open-issues.md))**:

현재 v5도 XP는 *누적 숫자 + 레벨업* 뿐. 무엇에 쓰는지·왜 모으는지·소진되는지는 미정. 이 결정은 *Duolingo 벤치마킹 금지* 제약 하에 별도 세션에서 독립 발상 필요 ([§13](./meta/open-issues.md)).
