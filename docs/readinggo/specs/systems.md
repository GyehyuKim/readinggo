...
> **v10.1 정합 (2026-07-16, #1176)**: #1086의 숫자 노출 폐지 결정은 유효하며 XP 적립·계산 계약은 바꾸지 않는다. 현행 상단바가 여전히 XP·Lv 숫자를 표시하는 것은 **코드/스펙 드리프트**이지 결정 번복이 아니다. 둥지 단계·성·스트릭의 컴팩트 표기는 **Telegram 디자인 검토 대기**이며, 승인 전에는 UX 코드를 변경하지 않는다. 승인 후 구현은 별도 코드 이슈/PR로 진행한다.
