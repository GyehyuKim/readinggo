...
> **v10.1 활성 결정 (2026-07-29, #1176)**: 홈 상단의 **XP·Lv 숫자 표시는 유지**한다. XP 적립·레벨 계산·둥지·성 관계는 그대로이며, 둥지 IA·성장 설명 UX는 별도 범위다.
> **v10.2 활성 결정 (2026-08-16, #1415)**: v10.1의 홈 상단 계약과 구 사용자 레벨 계산을 대체한다. 홈 상단은 **XP와 완성 둥지 수**를 `XP {n} · 🪺 둥지 {floor(totalXp / 1600)}개`로 표시하고 `Lv.` 숫자와 `floor(sqrt(totalXp / 100)) + 1` 계산/API를 제거한다. XP 적립·1,600 XP 주기·둥지 내부 단계 Lv1~5는 유지한다.
