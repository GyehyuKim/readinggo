...
| 가지·잎 | `user_books`·`sentences`의 실제 관계 | 정확한 가지 수·나뭇잎 수 | XP·좋아요·방문·연속일로 수량·크기를 보정하지 않음 |
...
- 2026-08-22 Hyu 결정으로 책나무 전환은 구 APK 호환을 유지하지 않는 큰 변화로 분류한다. 구버전의 XP API·컬럼 존재를 삭제 게이트로 사용하지 않는다.
- 코드 삭제는 `nest-grow` 내부 route와 책나무 화면, 책·문장·진도·세션, 최근 14일 리듬·누적 성장일을 보존해야 한다.
...
- 레거시 값은 새 책나무의 권위 데이터가 아니며, 삭제 전 백업 외의 신규 제품·분석 용도로 재사용하지 않는다.
...
3. 기록하지 않은 날에도 가지·잎·누적 성장일이 손상·감소하지 않는다.
...
> **v17 상태**: 아래 수치는 구 APK·기존 테스트의 동작을 설명하기 위해 보존한다. 새 기능·분석·책나무 계산에서 사용하지 않는다.
