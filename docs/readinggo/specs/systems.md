...
> **v17 목표 결정 (2026-08-19, #1452)**: 최근 14일 독서 리듬과 누적 성장일이 활성 제품 계약이다. 아래 §6.1~§6.3의 스트릭·방패·XP 수치표는 현행 코드와 구 APK 호환을 설명하는 레거시 as-built다.
...
| 단계 | 새 클라이언트 | 구 APK·DB 호환 | 완료 증거 |
|---|---|---|---|
| 1. 제품 폐기 | XP·둥지·성·방패·하루 만회 노출과 XP 소비/정렬 제거 | 기존 API·컬럼은 유지 | 신규 UI·분석에 XP 없음 |
| 2. 쓰기 동결 | 체크인·방문·좋아요·문장 일괄저장·외부 가져오기·완독에서 XP 호출 제거 | `increment_xp()` 등 legacy RPC는 값을 바꾸지 않는 no-op과 동결값 반환. 기존 `users.xp` 보존 | DB 쓰기·RPC 호출 로그에서 신규 버전 XP 증가 0 |
| 3. 참조 격리 | 책나무·성장일·KPI가 XP를 전혀 참조하지 않음 | `xp_earned`, `streak_broken`, `streak_repair*`는 `legacy_*` 분석군으로 격리 | 운영 리포트·대시보드 참조 제거 |
| 4. 물리 삭제 | 해당 없음 | **6-A**: 최소 지원 버전 강제, 지원 버전 전체 legacy 호출 0·운영/분석 참조 0·release cadence 기반 rollback 기간을 확인한 뒤 RPC→세션 XP 필드→사용자 XP 필드 순으로 migration | 삭제 계획·백업·rollback·Production 직접 검증 승인 |

- 현재 소비자 인벤토리에는 `nest.js` 체크인, `app.js` 방문/TopBar, 좋아요·반응, 배치 문장 저장, 외부 import, `datastore-supabase.js` 원자 RPC, `nest-grow.js`·`nest-theatre.js`·`ceremony.js`, 마일스톤·분석 이벤트가 포함된다. 구현 계획에서 이 목록을 코드 검색으로 다시 확정한다.
- 컬럼이나 RPC를 스펙 PR에서 삭제하지 않는다. 구 APK가 Supabase를 직접 호출할 수 있으므로 UI 숨김만으로 전환 완료로 간주하지 않는다.
- 레거시 값은 이행 감사와 롤백에만 사용하며 새 책나무의 권위 데이터가 될 수 없다.
...
4. 지원 중인 구 APK는 XP API·컬럼 존재 여부 때문에 크래시하지 않으며, 값은 동결된다.
5. 최소 지원 버전 미만은 업데이트 안내 후 XP legacy 경로에서 차단한다. 지원 버전 전체의 호출 0과 운영·분석 참조 0이 확인되기 전에는 물리 삭제 migration을 실행하지 않는다.
6. 삭제 날짜를 임의 기간으로 고정하지 않고 실제 release cadence·버전 수신·telemetry 증거로 정한다. UI에서만 숨긴 채 컬럼·RPC를 영구 유지하지 않는다.
