...
| 행동 | XP | 비고 |
|---|---|---|
| 일일 미션 완료 | **+10** | 하루 첫 세션만 |
| 짹 받음 (내 한 문장에 받은 좋아요) | **+1** | 일 최대 +20. 사회적 화폐 감각 |
...
레벨: `level = floor(sqrt(xp / 100)) + 1`.

**XP ≠ 둥지 ≠ 성**: 둥지 단계 = 활성 책 진척률([nest.md §5.2](./nest.md)), 성 = 완독 권수, XP = 행동 누적(상단바 숫자·레벨). 셋은 독립.
