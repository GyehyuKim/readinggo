...
레벨 계산값: `level = floor(sqrt(totalXp / 100)) + 1`(초반 쉽게·후반 더디게). 홈 상단에는 현재 XP와 이 계산값을 **XP {n} · Lv.{n}**으로 표시한다(#1176). 전체 XP와 레벨은 주기 완료 후에도 리셋하지 않는다.
...
- XP = 행동 누적(가중표 §6.3). 책과 무관하며 차감·리셋하지 않는다. 홈 상단의 XP·Lv 숫자 표시는 유지한다(#1176).
...
> **XP destination — 해소 (#1086, 2026-06-30; 노출 결정은 #1176으로 갱신)**: 구 "XP를 *무엇에 쓰는지* 미정"을 **둥지·성·서재로 해소** — XP가 둥지를 키우고, 둥지 주기가 성이 되고, 성·완독이 내 서재로 쌓인다. 그게 XP의 쓸모이며 별도 화폐·상점은 두지 않는다. #1086의 상단 숫자 폐지 부분만 대체되어, 홈 상단 XP·Lv 표시는 유지한다.
