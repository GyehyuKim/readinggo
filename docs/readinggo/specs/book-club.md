# 독서모임 탭 & 운영자 짹 — 화면 스펙

> **Split from** `docs/2. specifications/readinggo-spec.md` v6 (2026-05-28 분할). 원 위치: §5.5 (독서모임 탭, v6 마을 → 독서모임 전환), §5.6 (운영자 짹). 변경 이력은 git log 참조.
> **편집 정책**: 이 영역 변경은 이 파일 PR로. spec-only PR 룰 ([LF](../../1. research_and_lectures/lecture-frameworks.md#lf-week6-spec-only-pr)) 준수.

### 5.5 독서모임 탭

> **v6 전면 신설** — v5의 마을 탭(TBD)을 대체. T2 mini([§13.2](./meta/open-issues.md)) 및 마을 탭 재설계([§13.4](./meta/open-issues.md)) 안건 흡수 해소.

#### 독서모임 계층 구조

```
메가 스트림 (책 단위, 자동 생성)
  └─ 서브 모임 A: "팀 사피엔스 4주 완독" (기간 有, 비공개)
  └─ 서브 모임 B: "KAIST 독서 챌린지" (기간 有, 공개)
  └─ 서브 모임 N: ...
```

**메가 스트림**: 책 등록 시 자동 생성. 해당 책을 읽고 있는 모든 사용자의 최상위 커뮤니티. 가입 후 자유 참여. 구경 불가 — 가입자에게만 피드 공개.

**서브 모임**: 기간과 목표를 가진 소규모 챌린지 모임. 마감일까지 같이 달리는 레이스 구조.

#### 서브 모임 타입 × 접근 방식

| 타입 | 생성 주체 | 입장 방식 |
|---|---|---|
| **공개** (오픈월드) | 누구나 | 입장 신청 → 모임장 수락 |
| **비공개** (오픈월드) | 누구나 | 비밀번호 입력 or QR/초대 링크 |
| **독서모임 소속 공개** | 운영진 | 상위 독서모임 회원이면 자유 신청 |
| **독서모임 소속 비공개** | 운영진 | QR/초대 링크 |

**구경 불가**: 모임 피드·멤버 현황은 가입자에게만 공개. 공개 모임 목록·제목·인원만 비가입자에게 노출.

#### 독서모임 탭 메인 화면

```
┌─────────────────────────────────────────────┐
│ 📚 독서모임                    [+ 모임 만들기]│
├─────────────────────────────────────────────┤
│ 내가 참여 중인 모임                          │
│ ┌─────────────────────────────────────────┐ │
│ │ 팀 사피엔스 4주 완독   D-12  8명  ●●○●  │ │
│ └─────────────────────────────────────────┘ │
├─────────────────────────────────────────────┤
│ 지금 이 책을 읽는 사람들 (메가 스트림)       │
│ ┌─────────────────────────────────────────┐ │
│ │ 《사피엔스》를 읽는 중  234명             │ │
│ │ 인기 모이: "역사는 픽션이 만든 질서다"  │ │
│ └─────────────────────────────────────────┘ │
├─────────────────────────────────────────────┤
│ 공개 모임 둘러보기                           │
│ ┌──────────────────────┐ ┌────────────────┐ │
│ │ KAIST 독서 챌린지    │ │ 자기계발 4주   │ │
│ │ 《아주 작은 습관》   │ │ 《역행자》     │ │
│ │ 12명 D-8 [신청하기]  │ │ 5명 D-21 [↗]  │ │
│ └──────────────────────┘ └────────────────┘ │
└─────────────────────────────────────────────┘
```

#### 서브 모임 내부 화면

```
┌─────────────────────────────────────────────┐
│ 팀 사피엔스 4주 완독              D-12 남음  │
│ 유발 하라리 《사피엔스》 · 8명 읽는 중       │
├─────────────────────────────────────────────┤
│ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐        │
│ │🏰 ●  │ │🏠 ●  │ │🪹 ○  │ │🪵 ●  │        │
│ │@user1│ │@user2│ │@user3│ │@user4│        │
│ │72p   │ │58p   │ │31p   │ │15p   │        │
│ └──────┘ └──────┘ └──────┘ └──────┘        │
├─────────────────────────────────────────────┤
│ 오늘의 모이 하이라이트                       │
│ @user1: "역사는 픽션이 만든 질서다"          │
│ @user2: "인지혁명은 거짓말에서 시작됐다"     │
└─────────────────────────────────────────────┘
```

| 요소 | 규칙 |
|---|---|
| 마감일 카운트다운 | 모임 생성 시 설정한 종료일까지 D-N 표시 |
| 멤버 현황 그리드 | 불빛 ON(오늘 체크인) / OFF(미체크인). 페이지 진도 표시 |
| 완독 레이스 | 현재 페이지 기준 진도. 완독자에게 🏆 배지 |
| 모이 하이라이트 | 오늘 멤버들이 남긴 모이 최신 2~3개 |
| 격려하기 | 🪱 모이 보내기 (불빛 OFF 멤버에게만, 일 1회) |
| 스트릭 연동 | 모임 내 체크인은 개인 스트릭과 동일 기준 (책 무관 유저 단위) |

#### 서브 모임 생성 플로우

1. 책 선택 (내서재에서 또는 검색)
2. 모임 이름 · 기간(시작일~마감일) 설정
3. 공개/비공개 선택 → 비공개 시 비밀번호 or QR 자동 생성
4. 초대 링크 공유

#### Phase 배치

| 기능 | Phase |
|---|---|
| 메가 스트림 시드 데이터 (하드코딩) | 0 |
| 서브 모임 생성 · 참여 · QR/링크 | 1 |
| 비밀번호 모임 · 모임장 수락 플로우 | 1 |
| 완독 레이스 실시간 업데이트 | 2 |

친구+NPC 둥지를 그리드로 시각화.

```
┌─────────────────────────────────────────────┐
│ 🏘️ 리딩 빌리지                               │
│ 💡 불빛 ON = 오늘 읽음 · 🪱 모이 = 독려 알림  │
├─────────────────────────────────────────────┤
│ ┌────────┐  ┌────────┐  ┌────────┐          │
│ │🪱   ●  │  │🪱   ○  │  │🪱   ●  │  ← 3열 그리드
│ │  🏰    │  │  🏠    │  │  🪹    │
│ │@book_  │  │@gyehyu │  │@seung- │
│ │bear    │  │        │  │won     │
│ │ 🔥 64  │  │ 🔥 21  │  │ 🔥 0   │  ← Pixel 폰트
│ └────────┘  └────────┘  └────────┘
└─────────────────────────────────────────────┘
```

| 요소 | 규칙 |
|---|---|
| 둥지 아이콘 | 친구의 활성 책 진척 단계의 이모지 ([§5.2](./nest.md)) |
| 불빛 (●/○) | 오늘 세션 완료 → 골드 글로우 (`box-shadow: 0 0 8px gold`). 미완료 → `--ink-4` 회색 점 |
| 닉네임 | `@handle` |
| 스트릭 미니 | `🔥 N` (Pixel 폰트). streak 0이면 "쉼 중" 회색 텍스트 |
| 인용 1줄 | 오늘 작성한 문장 (없으면 미표시) |
| 액션 | 불빛 ON: "읽는 중" 배지 / 불빛 OFF: 카드 좌상단 🪱 클릭 → 토스트 |

**모이 보내기**:
- 카드 좌상단 🪱 클릭 → 토스트 `"@{handle}에게 🪱 모이를 보냈어요!"`
- 즉시 `.sent` 클래스 부여 → `opacity: 0.25`, `cursor: default`
- 새로고침 전까지 재전송 불가 (Phase 1: `pokes` 테이블 + KST 자정 리셋)
- NPC에게는 모이 못 보냄 (항상 자정 이후 곧 불빛 ON되니 자연스럽게 불가)
- **현재 정의**: 넛지(독려 알림)만. 수령자 효과 없음
- **방향성 미결**: 모이 N개 수신 → 방패 +1 등 게임 메카닉 연계 가능. Phase 1 설계 시 결정

**받은 모이 섹션** (하단 별도 카드):
- "@{handle}가 모이를 보냈어요" + "오늘도 짹 한번! 같이 읽자🐦" + 시간 표기

**친구 상세 시트**:
- 마을 카드 탭 → 바텀시트
- 표시: 닉네임, 스테이지, NPC 여부, bio, 읽고 있는 책 리스트 (칩), 오늘의 문장
- 책 칩 탭 → [§5.8](./profile.md) 책 상세 페이지 (내서재 탭)

**마을 게시판**:
- 마을 주민(친구+NPC)이 올리는 자유 감상 게시글
- 소셜 피드(문장 공유)와 차이: 게시판은 자유 텍스트, 피드는 sentences 테이블 기반
- Phase 0: NPC 시드 게시글 3개 하드코딩. Phase 1: 텍스트 입력 → 저장

### 5.6 운영자 짹 (Operator) — v5 신설

첫 7일 보호의 *주축* 메커니즘. NPC 시뮬레이션이 아니라 *실제 사람(운영자)* 이 사용자가 막 입력한 모이에 짹/박수로 응답.

#### 5.6.1 운영자 정의

- `users.is_operator` bool 컬럼 신설 (`is_npc` 와 구분)
- 운영자 = 학기 데모 기간 동안 계휴 본인 + 팀원(승원·윤지) 중 권한 부여된 계정
- UI 표식: 닉네임 옆 **✨** 이모지 (NPC 의 ` · NPC` suffix 와 다른 표식)
- 일반 사용자 마을 그리드·소셜 피드에서는 *조금 더 자주 등장하는 친숙한 친구* 처럼 보이는 정도. 운영진이라는 사실은 굳이 강조 안 함

#### 5.6.2 운영자 짹 스케줄 (첫 7일)

| 일자 | 트리거 | 내용 |
|---|---|---|
| D1 | 첫 모이 입력 직후 (자동 큐잉) | 환영 짹 — *"시작이 가장 어려운 부분이었어요. 내일도 한 줄 기다릴게요."* |
| D2~D6 | 매일 사용자 모이 작성 시 운영자 대시보드에 알림 | 사용자 한 문장을 *진짜로 읽고* 짧은 응답 짹 작성. 응답 시간 24h 이내 목표 |
| D7 | 7일 연속 도달 시 자동 큐잉 | 졸업 짹 — *"일주일을 채웠어요. 여기서부터가 진짜 ReadingGo예요."* 둥지 진화 🏠 트리거와 동시 |
| 미참여일 | 운영자 짹 일시정지 (스트릭 끊김 시) | 첫 복귀일에 부드러운 환영 짹 1회 — *"다시 펼쳐주셔서 고마워요"* |

#### 5.6.3 운영자 대시보드 (Phase 1+)

별도 화면 `/operator` (운영자 권한 사용자만 접근):

- 미응답 큐: 최근 입력된 사용자 모이 중 운영자 짹이 아직 안 달린 것
- 각 항목에 사용자 닉네임·책·페이지·모이 텍스트·가입 후 일수 표시
- "응답" 버튼 → 미니 작성 폼 (200자) → 즉시 짹 발송
- 빠른 응답 템플릿 5개 (환영·격려·공감·인용 호응·졸업)

Phase 0: 시드 응답 5개 하드코딩, 사용자가 첫 입력하면 0.5초 지연 후 자동으로 받은 짹 토스트.

#### 5.6.4 데이터 모델 ([§7.3](./backend.md) 확장)

- `users.is_operator` bool DEFAULT false
- `operator_replies` 테이블:
  - `id`, `to_sentence_id`, `from_user_id`(운영자), `text`, `created_at`
- 일반 짹(`claps`)과 분리. 운영자 응답은 카드 UI에서 *별도 영역* 으로 두드러지게 표시

#### 5.6.5 자동화 점진 계획 ([§13](./meta/open-issues.md) 안건)

학기 데모 N≤20 에서는 수동. 사용자 증가 시 자동화로 점진 전환 — 구체 임계값·UX는 학기 후 결정. 리스트만 [§13](./meta/open-issues.md)에 보존.

#### 5.6.6 운영자 짹 vs NPC

|  | 운영자 짹 | NPC |
|---|---|---|
| 응답 시점 | 사용자 모이 *내용을 읽고* 24h 이내 | 자정 배치, 임의 추첨 |
| 응답 텍스트 | *사용자 한 문장에 맞춤* | 사전 시드 풀에서 추첨 |
| 등장 빈도 | 첫 7일은 매일 1회 이상 보장 | 자율적 |
| 사용자 인식 | "사람이 진짜 봤다" | "활동 중인 친구가 있다" |
| 표식 | ✨ | NPC suffix |

---

