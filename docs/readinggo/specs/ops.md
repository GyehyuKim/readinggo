...
책 public/private·전체 문장/생각 상속은 [backend.md §7.0.1–2](./backend.md)의 별도 안전 컷오버다. 과거 친구 공개 확대안은 대체됐으며 legacy 값은 보수적 이관 자료일 뿐이다. spec merge → 구현 PR → DEV DB/client/RLS 및 공유 실패·철회·기존 기록 비손실 검증 → 검증된 동일 SHA Production 승인 순서를 지킨다. 기존 특정 소유자의 문장 공개 승인은 책 전체 생각 공개까지 포함하는지 운영자가 확인하고 동의 증거를 기록한다. 이 문서 변경은 실사용자 데이터를 변경하는 실행 승인이 아니다. Production/스토어 작업은 담당자의 기존 게이트를 그대로 따른다.
