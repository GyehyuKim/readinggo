...
> **기준**: 이 디렉터리는 ReadingGo의 살아있는 SSOT다. 충돌 시 `meta/decisions.md`의 가장 최신 활성 결정 → 해당 기능 스펙 → 현재 구현·검증 증거 순으로 정합하며, 과거 v5~v17 설명은 감사 이력이지 현재 계약이 아니다.
...
- **책 공개 SSOT (#1619)**: 데이터·이관=`backend.md §7.0.1–2`, 노출=`feed.md §5.7.1`, 책 설정=`profile.md`, 공유 확인·재개=`share.md §1.1`, story source 권한=`reading-story.md §6`, 에이전트 재사용 안내=`legal-copyright.md §4.4`. 구 문장 3단계·생각 별도 privacy·계정 기본값과 미병합 public-only 퇴역안은 대체한다. 계정/Auth·읽기방·개인 AI·moderation·위시는 별도 경계다.
...
| 등록해 읽기 시작한 책 | **책** | `user_books`가 권위. 소유자별 공개 단위이며 전역 `books`와 구분 |
| 기록 공개 상태 | **책 공개 설정** | `user_books.visibility=public\|private`, 신규 private. 모든 현재·미래 문장·내 생각 상속 |
| 중단한 책 | **중단한 책** | 저장 status `aborted` 유지. 포함·제외 필터 제공 |
| 찜한 책 | **읽고 싶은 책** | `wish_books` 유지. 실제 읽기 시작 시 `user_books`로 전환 |
...
| 읽은 책과 마음에 남은 문장을 한눈에 되찾는다 | 네 상태의 책을 하나의 서재 흐름에서 보고 검색·필터·정렬·상세로 정확히 탐색하게 함 |
| 혼자가 아니라 같이 읽는다 | 공개 책의 문장·내 생각을 로그인 없이 읽고, 로그인 후 상호작용하며 읽기방에서 함께 읽는 경험 제공 |
| 내 기록의 공개를 책마다 정한다 | 새 책은 비공개, 공유 시 책 전체 현재·미래 문장·생각 공개를 확인하고 저장 성공 후 원래 공유를 이어감. 다시 비공개로 전환 가능 |
...
[현재 페이지와 마음에 남은 문장 저장]
   ↓
[해당 책에 문장과 진도 저장 + 최근 독서 리듬·누적 성장일 갱신]
   ↓
[서재에서 책·문장 탐색 / 공개 문장은 친구와 공유]
   ↓
[원할 때 같은 자리부터 다시 펼치기]
```

**목표 페이지 설정 없음.** 한 번 쉬었다 돌아와도 기존 책·문장·진도는 사라지거나 손상되지 않는다.

### 2.1 핵심 기록 트리거

**v18 유지 — 책에서 마음에 남은 문장 저장.** 문장 원문은 공개범위와 무관하게 최대 1,000자이며 글자 수 때문에 공개범위를 강제 변경하지 않는다. 현행 공개 저장은 200자 경계와 길이 기반 `private` 강제가 남아 있고 #1457 구현 후속이다. 누적 저작권 위험의 현재 수용·추후 검토와 출처표기·삭제요청·seed provenance 의무는 `legal-copyright.md`를 따른다. 페이지 진도와 문장은 `user_book_id`에 연결하며 정확히 되찾을 수 있어야 한다.

### 2.2 이탈 방어선

- **사라지지 않는 기록 자산** — 책·문장·진도·누적 성장일은 휴식이나 스트릭 단절로 감소하지 않음
