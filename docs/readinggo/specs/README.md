...
- 책 데이터 canonical = Supabase `books`(#490) — 게스트도 anon RLS read. 구 정적 `books.tsv`는 제거됨(#972); 폴백은 인라인 `RG_BOOKS`(12)
...
- **책 데이터 단일 소스 = Supabase `books` (#490 결정 2026-06-15, Phase 1 SSOT)**:
  ...
  - 구 정적 `books.tsv`는 **제거됨(#972, 본 #490 후속)**. 폴백은 인라인 `RG_BOOKS`(12) **최소치**뿐(네트워크/부팅 실패·Supabase 미설정 시 데모 무중단).
  - **코드 구현 완료 (#490, A: isbn13 매칭; TSV 제거 #972)**: `loadBooks` 는 **Supabase `books` 우선(anon RLS read) + isbn13 매칭 + 인라인 RG_BOOKS 폴백**으로 구현. 게스트도 publishable key 로 같은 카탈로그를 읽고, 부팅 시 `getBook` 동기 캐시(id+isbn13 인덱스)를 워밍한다. Supabase 비었거나 네트워크/부팅 실패면 인라인 12권으로 무중단. **books 데이터 적용(#528 알라딘 백필)**만 별도(live DB).
