...
- **책 데이터 단일 소스 = Supabase `books` (#490 결정 2026-06-15, Phase 1 SSOT)**:
  ...
  - **코드 구현 완료 (#490, A: isbn13 매칭)**: `loadBooks` 는 **Supabase `books` 우선(anon RLS read) + isbn13 매칭 + TSV 폴백**으로 구현. 게스트도 publishable key 로 같은 카탈로그를 읽고, 부팅 시 `getBook` 동기 캐시(id+isbn13 인덱스)를 워밍한다. Supabase 비었거나 네트워크/부팅 실패면 정적 TSV 로 무중단. **books 데이터 적용(#528 알라딘 백필)**만 별도(live DB).
