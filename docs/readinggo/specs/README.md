...
| 그룹 기능 | **마을** | ❌ "독서모임 / 메가스트림 / 서브모임" |
| 둥지 진행 시각화 | **둥지가 자란다** (활성 책 진척률 5단계) | ❌ "The Path / 세션 노드" |
| 누적 성취 | **성(🏰) 컬렉션** (= 완독 권수, 파생) | v7 신설 |
| 하단 탭 | **둥지 / 마을 / 소셜 / 프로필** | ❌ "독서모임 / 내서재" |
...
| 혼자가 아니라 같이 읽는다 | 단방향 팔로우 + 마을 둥지 시각화 + 짹 + NPC 동행 |
...
[일일 미션: 활성 책의 현재 페이지 입력 + 한 문장 입력(둘 다 강제)]
   ↓
[스트릭 갱신 → XP 보상 → 둥지가 자람(진척률) / 완독 시 🏰 1개]
   ↓
[마을에서 친구 둥지 불빛 ON, 소셜 피드 노출 → 짹]
   ↓
...
> ⚠️ **v7 재작성 예정.** 아래 시나리오와 §10 오픈 태스크는 v6 용어(모이·The Path·마을 그리드·민트 리그·운영자 짹)를 그대로 담고 있어, 피처 spec 재작성과 함께 갱신한다. v7 정본 용어는 §0.5, 결정은 [decisions.md §8.0](./meta/decisions.md).
