...
> **v18 목표 결정 (2026-08-25, #1515)**: 책나무 시각화·탭 아이콘·잎 생성 모션은 보류한다. 서재는 빠른 좌우 flick에서도 스쳐 지나가는 표지가 준비되어야 하며, 상태를 색만으로 구분하지 않고 제스처 외 조작과 `prefers-reduced-motion`을 제공한다. preload/window 수치와 최종 모션은 성능·접근성 검증 전 확정하지 않는다.
...
## 10-v17 책나무 시각·상호작용 계약 — 보류 이력

### 10.1 정보 밀도

- 나무 개요는 전체 가지·잎의 분포와 성장 감각만 전달한다. 모든 책 제목·문장 잎을 동시에 렌더하지 않는다.
- 정확한 수는 헤더(`가지 N · 나뭇잎 M`)와 선택한 가지 상세에서 텍스트로 제공한다.
- 검색·상태 필터·문장 목록은 나무 그림에만 의존하지 않는 정확하고 접근 가능한 목록/상세 경로로 제공한다. 렌더링 기술과 정렬 방식은 구현 계획에서 정한다.
- 모바일에서 수백 권·수백 문장을 가진 사용자도 나무 요약·검색·상태 필터·가지 상세를 사용할 수 있어야 한다. 검증 fixture의 정확한 규모, 대상 기기와 성능 합격 수치는 구현 계획에서 측정·승인한다.

### 10.2 상태 표현

| 상태 | 시각 언어 | 금지 |
|---|---|---|
| 읽는 중 | 살아 있는 현재 가지와 상태 라벨 | 조급함·진도 뒤처짐 경고, 문장 저장 없이 잎 생성 |
| 완독 | 유지되는 충실한 가지와 상태 라벨 | 트로피 랭킹·대형 진화 폭발 |
| 중단 | 쉬어가는 가지와 닫힌 새눈 | 마른/부러진 가지, 낙엽, 시듦 |
| 관심 | 나무 주변의 새 가지 후보 | 실패·미완료 압박 |
| 독서 공백 | 손상 없이 그대로 남은 나무 | 스트릭 단절·상실 예고·죄책감 |

색상만으로 상태를 구분하지 않는다. 상태명·아이콘·텍스트를 함께 제공하고, 터치 영역은 최소 44×44px, 동적 텍스트 확대와 스크린리더 순서를 지원한다.

### 10.3 모션·저장 피드백

- 문장 저장 성공 뒤 선택 가지에 잎 한 장이 더해지는 짧은 인라인 변화만 제공한다.
- `prefers-reduced-motion` 또는 앱 저모션 설정에서는 교차 페이드 또는 즉시 갱신으로 대체한다.
- 화면 전체를 막는 레벨업, 성 완성, 폭죽, 흔들림, 자동 카메라 이동은 사용하지 않는다.
- 저장 실패 시 잎을 선반영하지 않고 원래 상태와 재시도 가능한 오류를 유지한다.

### 10.4 아이콘·캐릭터 게이트

- 하단 `책나무` 아이콘은 Lucide 공식 `book-open`·`sprout`·`tree-deciduous`를 20px·24px, 활성/비활성, light/dark, 실제 5탭 조건으로 #1453에 팀 공유했으며 **최종 선택 보류**다. Android 실기기와 팀 의견 뒤 1개를 결정한다.
- 선택 전 임시 이모지나 특정 후보 SVG를 제품 정본으로 선반영하지 않는다. 선택 결과는 `meta/decisions.md`와 `README.md` 용어 표에 함께 기록한다.
- 캐릭터는 진화 없는 2D 참새를 유지한다. 이름은 팀 결정 보류이며 Judy·Jerome 논의와 비교 검증 전까지 `Jacky / 재키`를 유지하고 새 이름을 카피·분석 이벤트·접근성 이름에 하드코딩하지 않는다.
...
**활성 의미체계**: 브랜드 민트와 접근 가능한 중립 토큰을 우선한다. 아래 불꽃 오렌지·골드 토큰은 현행 스트릭·XP 표면의 레거시 as-built이며 신규 책나무 의미나 보상 규범으로 확정하지 않는다.
...
참새 마스코트: 현행 온보딩·소개 화면·라벨은 **SparrowMark SVG**(`<window.SparrowMark>` / 인라인용 `<window.SparrowInline>`, `sparrow-bounce` 애니메이션)로 표현한다. (#785/#823/#824 — 🐦 이모지를 브랜드 SVG 마크로 통일.) v17에서도 진화 없는 2D 참새 방향은 유지하지만 최종 표시명·배치·모션은 미확정이다. `NEST_STAGES` 이모지(🪵🪹🏠🏡🏰)는 레거시 둥지 구현 설명일 뿐 활성 책나무 규범이 아니다.
