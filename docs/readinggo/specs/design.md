...
| 성격 포인트 (감정·마스코트) | 참새는 SparrowMark SVG, 그 외 이모지 유지 | 참새 마크(SparrowMark), 🔥 스트릭 수치, 세리머니 강조 |
...
> ⚠️ **표 STALE — 카피 리프레시 필요 (드리프트 정정 2026-07-09)**: 아래 마이크로카피 표는 현행 코드 CTA와 상당수 어긋난다(예: 메인 CTA "오늘의 한 쪽, 짹 하기"·마을/모이/The Path 관련 문구가 코드의 "읽을 책 등록하기"/"저장하기"/"남기기" 등과 불일치, 폐기된 마을 탭 문구 잔존). 개별 행을 즉시 재작성하지 말고 **별도 카피 정합 패스**에서 일괄 갱신할 것 — 이 표를 카피 SSOT로 신뢰하지 말 것.
...
> **독서 동반자 호칭 = '재키' (#684, 2026-06-17)**: LLM 독서 파트너(NPC) 캐릭터의 한국어 호칭은 **'재키'**(영어 이름 Jacky 유지). 동반자 진입·대화 UI는 모두 '재키' — 예: `재키와 대화하기`, `재키와 대화 이어가기`, 모달 헤더 `재키`(부제 '독서 동반자'), `오늘 재키랑 깊이 이야기했네요`.
> **단, 액션 동사 '짹'은 보존**(브랜드 자산 — 1단어 다의 분리): 위 표 행 1·3·11의 `"오늘의 한 쪽, 짹 하기"`·`"오늘의 짹"`·`"내일도 짹 →"`, 그리고 `짹 등록하기`·`한 줄을 짹 해보세요`(= 한 쪽 기록 = 트윗처럼 "지저귀다") 는 그대로 둔다. 리액션 '좋아요'(구 짹, #641)와도 별개. 결정 비준: [meta/decisions.md §8.12](./meta/decisions.md).
