...
> **핵심 결정**: 텍스트·파일 export가 있는 소스는 **붙여넣기 / 파일·엑셀 업로드 → 텍스트 LLM 파싱**이 공동 1순위, **스크린샷 + 비전**([integrated-shelf.md §4](./integrated-shelf.md) / #772)은 *글자를 못 뽑는 좁은 소스(도서관 대출이력 등)*용 폴백으로 강등(#1039 A 조사).
...
> **구현 상태 (2026-06-29, P1 머지 예정)**: 붙여넣기/파일(.csv·.tsv·.txt) → `POST /api/parse-books`(텍스트 LLM) → `RG_shelfImport.extractBooksFromText` → `matchRows` → `TextImportModal` 검수(상태 칩·목적지 1회 선택) → `importStaging.add` 검토함까지 **코드 구현 완료**(#1039 P1). **`.xlsx` 파일 파싱은 제외** — 엑셀은 셀 복사→붙여넣기(TSV)로 커버, 파일 직접 파싱은 후속(라이브러리 결정 필요 — Stack Lock §6). 매칭·검수·검토함 코어는 [integrated-shelf.md](./integrated-shelf.md)(`RG_shelfImport`·`DataStore.importStaging`)를 그대로 재사용, 입력 어댑터(텍스트/파일)와 별도 모달(`TextImportModal`)만 신설(열린 PR #1078 의 `ShelfImportModal` 무수정 — 충돌 회피). [integrated-shelf.md](./integrated-shelf.md)는 참조만(편집 안 함).

## 1. 목적·전략

- **문제**: 헤비 독서러는 이미 어딘가(노션·엑셀·블로그·서점 구매내역)에 도서기록을 쌓아 뒀다. 그걸 ReadingGo로 옮기는 마찰이 크면 빈 서가로 시작 → 이탈.
- **왜 텍스트 1순위 (비용·정확도 근거)**: 텍스트가 있는 소스를 스샷으로 받으면 비전(Gemini) 호출이 들고(LLM 코스트↑), 표지 아트·UI·잘린 글자 때문에 회수율이 출렁인다. **실측([integrated-shelf.md §4.2](./integrated-shelf.md)): 표지 그리드 텍스트 OCR 0권 / 풀이미지 단일 비전 호출 1권 — 같은 정보가 텍스트로 있으면 LLM 파싱이 더 싸고(텍스트 토큰만) 더 정확(원문 그대로 읽음)**하다. 그래서 **텍스트·파일 소스 = 붙여넣기/파일 업로드 1순위, 스크린샷 = 좁은 폴백**으로 재배치한다.
...
## 3. 입력 (붙여넣기 텍스트 + 파일/엑셀 = 공동 1순위)

- **1순위 ⓐ — 붙여넣기 텍스트**: 노션·메모·웹 책장(밀리·왓챠피디아 등 웹은 제목이 선택 가능한 텍스트) 등에서 **복사한 임의 포맷 텍스트**를 큰 `textarea`에 붙여넣는다. 줄바꿈·탭·표·머리말·합계 행이 뒤섞여도 받는다("개떡같이 넣어도 찰떡같이").
- **1순위 ⓑ — 파일 업로드**(#1039 A 조사 반영): 주요 서점이 구매·소장 내역을 **구조화 파일로 export** — **알라딘·예스24 = 구매내역(`.xlsx`/`.csv`)**, **리디 = 구매목록 파일**. 정렬된 표라 붙여넣기보다 *더 깨끗*. **P1 구현**: 클라가 **`.csv`·`.tsv`·`.txt`** 를 `file.text()` 로 읽어 텍스트로 평탄화 → 같은 `/api/parse-books`로 흘린다(2MB 컷). **`.xlsx` 는 P1 제외** — 새 파싱 라이브러리(SheetJS 등)가 필요해 Stack Lock 결정 대상이라, **엑셀은 셀을 복사→붙여넣기(표가 TSV로 들어옴)로 커버**하고 `.xlsx` 파일 직접 파싱은 후속(P2, 라이브러리 결정 동반). 노션 export(`.zip`/`.md`)도 후속.
- **진입**: 내서재 빈 상태 CTA + 임포트 메뉴에서 "붙여넣기 / 파일로 가져오기"(스샷 복원보다 **위**에). 스샷 경로(#772)와 형제 진입점이되 **텍스트/파일을 먼저** 권한다.
...
> **구현됨 (P1)**: `worker/index.mjs` 에 `parseBooksProxy` + `FLEXIBLE_PARSE_SYSTEM` 추가(`/api/wiki-ask` 형제, 동일출처 가드). 입력 `{ text }`(32KB 컷), `callLLM`(temperature 0.1, maxTokens 4000), 파서는 `parseShelfBooks` 확장(아래 `normImportStatus` 로 status 정규화 + date passthrough), 출력 `{ books, partial? }`, 미설정 시 `503 { demo:true }`.
...
- **출력**: `{ books, partial?, empty? }`. `status`는 `completed`/`reading`/`wish`로 정규화(매핑 못 하면 생략 → 검수 시 사용자가 목적지 선택, §8).
...
- **상태**(`status`): 데이터에 있으면 그대로 목적지 라우팅(`addBatch` status). **없으면 임의 기본값 없이** 검수 시 사용자가 목적지를 1회 선택(§8 결정).
...
- **재배치(임포트 1순위 = 텍스트/파일, #1039 A 조사)**: 텍스트·파일 export가 있는 소스(노션·웹 책장·서점 구매내역·엑셀)는 본 경로가 1순위. **A 실태조사 결론**: 캡쳐를 *막는* 앱(밀리·리디·교보 sam = `FLAG_SECURE`)은 동시에 웹/파일 export가 있어 "스크린샷이 막힌 곳"과 "필요할 곳"이 겹치지 않는다 → **스크린샷+비전이 꼭 필요한 소스 ≈ 도서관 대출이력(책이음 단편화) 하나.** 스크린샷은 그 **좁은 폴백으로 강등** + **추가 투자(타일링 고도화 등) 중단**. UI는 텍스트/파일을 먼저 권한다.

## 8. 결정·열린 결정 (계휴 확정 필요)

- **[결정]** 입력 1순위 = **붙여넣기 텍스트 + 파일/엑셀 업로드(공동)**. 알라딘·예스24·리디 export가 가장 깨끗(§3, #1039 A 조사). 노션 export만 후속.
- **[결정]** 매칭·검수·등록·검토함 = [integrated-shelf.md](./integrated-shelf.md) 코어 **재사용**, 입력 어댑터만 신설(§7). 텍스트 LLM = 기존 프록시 재사용·키 서버 보관(§6).
- **[결정 — #1039] 상태 없을 때 목적지 = 사용자 1회 선택(기본값 없음)**: 파싱이 행별 `status`를 못 주면 **임의 기본값을 넣지 않는다**. 검수 진입 시 목적지(읽은책/읽고싶어요/읽는중)를 사용자가 **명시적으로 1회 선택**해야 적재 가능(미선택 시 "담기" 비활성). 근거: 쌓인 기록은 '읽은 책'이 많지만 위시·읽는중도 섞여 `completed` 자동가정은 오등록 위험 → 사용자 의도를 강제 확인(파싱이 `status`를 준 행은 그 값을 기본 선택, 안 준 행만 선택 요구).
...
| Phase | 범위 | 리스크 |
|---|---|---|
| **P1 (MVP) — ✅ 구현됨(2026-06-29)** | 붙여넣기 텍스트 **+ 파일 업로드(`.csv`·`.tsv`·`.txt`)** → `/api/parse-books` → 매칭·검수(`TextImportModal`: 상태 칩·목적지 1회 선택·별점) → 검토함 적재·이동. 상태·별점 보존. **`.xlsx` 파일 파싱은 제외**(셀 복붙으로 커버) | 낮음(코어 재사용) |
| **P2** | **`.xlsx` 파일 직접 파싱(라이브러리 결정 동반)** · 날짜 보존(스키마 얇은 확장, §5.1·§8 ②) · 큰 입력 청크 분할(§4.2) · 노션 export 파싱 | 낮음~중 |
| **P3** | 좁은 스크린샷 폴백(도서관 대출이력 등) 정합 — 필요 시 | 중 |
...
- [ ] 상태·별점 보존(데이터에 있으면 그대로, 없으면 §8 사용자 1회 선택) · 날짜는 스키마 있을 때만(§5.1)
