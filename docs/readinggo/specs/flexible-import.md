...
- **1순위 ⓑ — 파일/엑셀 업로드**(#1039 A 조사 반영): 주요 서점이 구매·소장 내역을 **구조화 파일로 export** — **알라딘·예스24 = 구매내역 엑셀(`.xlsx`/`.csv`)**, **리디 = 구매목록 파일**. 정렬된 표라 붙여넣기보다 *더 깨끗*. 클라가 CSV·TSV·엑셀을 텍스트로 평탄화해 같은 `/api/parse-books`로 흘린다(잘 정형된 표는 LLM 생략·직파싱도 가능). 노션 export(`.zip`/`.md`)는 후속.
...
| Phase | 범위 | 리스크 |
|---|---|---|
| **P1 (MVP)** | 붙여넣기 텍스트 **+ 파일/엑셀 업로드(알라딘·예스24·리디 export)** → `/api/parse-books` → 매칭·검수(목적지 선택·별점) → 검토함 적재·이동. 상태·별점 보존 | 낮음(코어 재사용) |
| **P2** | 날짜 보존(스키마 얇은 확장, §5.1·§8 ②) · 큰 입력 청크 분할(§4.2) · 노션 export 파싱 | 낮음~중 |
