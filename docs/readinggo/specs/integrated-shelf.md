...
**컨셉 교정 (2026-06-18, 계휴)**: 1차 구현이 LLM으로 블로그 글을 **요약·재작성**해 "독자가 안 쓴 문장"을 만들어 둔 게 발견됨(감사 65건 중 진짜 독자 글 5건 미만, 출판사 소개·평론·광고 다수). **요약·재작성도 '가짜'의 일종**이다. → LLM의 역할을 **원문 발췌·선별·필터로 한정**(새 문장 생성 금지), **발췌 검증**으로 강제. 소스도 블로그 개인 후기 + 알라딘 독자 리뷰로 넓힘(§5.4). 기존 시드는 전량 재생성.
...
- 동작:
  ...
  3. 부족하면 `need`개만큼 수집 → **독자가 쓴 한 문장을 원문 그대로 발췌**(LLM은 선별·필터만 — **재작성·요약 금지**, #774 컨셉 교정 2026-06-18) → **새 시드 `seed_sentences` insert**(중복 url 스킵) → 누적 반환.
     - **소스(a+b)**: ⓐ 네이버 블로그 **개인 후기**(`읽고`/`독후감`/`후기`/`감상`/`한줄평` — 구 `책`/`서평`은 광고·출판사 소개 유입원이라 **폐기**) + ⓑ 알라딘 **독자 리뷰 제목**(`reviewList` title; 리뷰 본문은 API 미제공이라 제목만, `seedAladinReviews`).
     - **버림**: 출판사 책 소개·줄거리 요약·평론조 단정(`~한 작품이다/명저다`)·광고·홍보·별점/방문자수/해시태그 군더더기·책 무관 글·동명 영화/드라마. 쓸 만한 원문 감상이 없으면 **0건**(가짜로 채우지 않음 — LLM 미작동 시에도 raw 스니펫 저장 금지).
     - **발췌 검증**: LLM 출력이 입력 원문에 실제로 존재하는지 확인(공백 제거 후 앞부분 포함). 재작성/창작이면 버림 → "독자가 실제로 쓴 글"만 남김.
