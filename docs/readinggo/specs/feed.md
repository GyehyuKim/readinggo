...
> **v18 목표 결정 (2026-08-25, #1515)**: 하단 목적지는 `같이읽기`이며 피드는 그 안의 보조 표면이다. 책·사용자 랭킹과 XP 보상을 제거하고 문장 공개범위를 서버 권한대로 표시하는 계약은 유지한다. 폐기 예정 친구 기능 진입·자동 활성화는 보류한다.
...
- 친구 카드에서 레거시로 이동하지 않는다. 기존 폐기 예정 전용 RPC/RLS는 즉시 삭제하지 않고 보안 검증·향후 재검토를 위해 fail-closed 상태로 보존한다.
- 신고·차단·운영자 숨김은 읽기방·피드의 모든 표면보다 우선한다.

#### 수용기준

1. 신규 버전에서 피드 진입·좋아요·방문으로 XP나 독서 활동일이 증가하지 않는다.
2. 랭킹 카피·순위 숫자·경쟁적 정렬과 폐기 예정 친구 기능 진입점이 렌더되지 않는다.
