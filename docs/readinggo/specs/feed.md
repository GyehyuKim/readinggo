...
| 책 상세 높이·인기 한 문장 (#594) | `BookInfoModal` 시트는 **`max-height: 90vh` 내 스크롤**(정보가 길어도 충분히 노출). 하단에 **🔖 이 책의 한 문장 Top5** 섹션 — 그 책의 **공개 한 문장 중 짹(좋아요) 많은 순 최대 5개**. 데이터 `sentences.byBook(bookId, {limit:5, sort:'likes'})` ([backend.md](./backend.md)). 카드: 작성자(핸들/이름)·문장·페이지·**짹 수(읽기 전용 — 타인 문장이라 짹 토글·대화 버튼 없음)**. 본인 문장 제외(`neq user_id`). **Phase 0 게스트·비UUID 책 id·빈 결과 → 섹션 생략**(빈 노이즈 방지). "왜 읽는지"의 사회적 맥락 제공 |
...
| 맥락 | 공개범위 | 책갈피(🔖) | 좋아요 | 수정/삭제 |
|---|---|---|---|---|
| **내 한 문장** | ✅ 순환 토글(전체공개→친구공개→비공개) | ✅ | ✅ | ✅ 본문 수정·삭제 |
| **타인 한 문장** | — | ✅ | ✅ | — |
...
**리액션 표면 카피 통일 = '짹' (#616)** — 리액션의 **정본 표면 용어는 '짹'**. 같은 카드에 '좋아요'와 '짹'이 혼재하던 드리프트를 제거한다.

- UI 노출 라벨 `좋아요`·`좋아요 N` → **`짹`·`짹 N`** 으로 통일(개념 정의 "좋아요 = 짹"은 §5.7 리액션 행 유지 — **표면 카피만** '짹').
- 내부 식별자·`sentences.likes` 컬럼명·핸들러명은 변경 불필요(표면 카피 한정).
