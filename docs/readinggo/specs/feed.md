...
> **v18 목표 결정 (2026-08-25, #1515)**: 하단 목적지는 `같이읽기`이며 피드는 그 안의 보조 표면이다. 책·사용자 랭킹과 XP 보상을 제거하고 문장 공개범위를 서버 권한대로 표시하는 계약은 유지한다. 친구 책나무 진입·자동 활성화는 보류한다.
...
- 문장 카드는 `sentences_public`의 viewer별 결과만 렌더한다. `public`은 허용 범위, `followers`는 상호 팔로우 친구, `private`는 본인만 본다.
- 친구 카드에서 책나무로 이동하지 않는다. 기존 friend-tree RPC/RLS는 즉시 삭제하지 않고 보안 검증·향후 재검토를 위해 fail-closed 상태로 보존한다.
- 신고·차단·운영자 숨김은 읽기방·피드의 모든 표면보다 우선한다.

#### 수용기준

1. 신규 버전에서 피드 진입·좋아요·방문으로 XP나 독서 활동일이 증가하지 않는다.
2. 랭킹 카피·순위 숫자·경쟁적 정렬과 친구 책나무 진입점이 렌더되지 않는다.
...
### 5.7.0-v17 같이읽기 내 피드 계약 — 보류 이력

- 화면 제목과 하단 탭 라벨은 `같이읽기`다. 내부 키 `social`과 파일명 `feed.md`는 구현 호환을 위해 유지할 수 있다.
- 피드의 역할은 공개가 허용된 문장과 읽기방 활동을 시간·관계 맥락으로 보여주는 것이다. `Top 5`, `1위`, 인기 순위, 리더보드, 경쟁적 백분위는 사용하지 않는다.
- 좋아요는 반응일 뿐 XP·가지 성장·나뭇잎 수를 만들지 않는다.
- 문장 카드는 `sentences_public`의 viewer별 결과만 렌더한다. `public`은 허용 범위, `followers`는 상호 팔로우 친구, `private`는 본인만 본다.
- 친구 카드에서는 `책나무 보기`로 이동할 수 있다. 비친구·차단 사용자는 직접 URL·API로도 친구 책나무를 볼 수 없다.
- 공개 문장이 0개인 가지도 책 상태 공개가 승인된 친구에게는 가지로 보일 수 있다. 이때 viewer에게 허용된 문장 수만 표시하고 private 문장의 존재·개수·clap/report 상태·오류 차이를 암시하지 않는다. 숨겨진 문장 ID에 대한 직접 clap/report/count 요청은 존재하지 않는 ID와 구분되는 신호를 주지 않는다.
- 신고·차단·운영자 숨김은 기존 §5.7.4 계약을 그대로 적용하고 모든 책나무·읽기방 표면보다 우선한다.

#### 수용기준

1. 신규 버전에서 피드 진입·좋아요·방문으로 XP가 증가하지 않는다.
2. 랭킹 카피·순위 숫자·경쟁적 정렬이 렌더되지 않는다.
3. 차단·비친구·문장 공개범위 조합을 RLS와 E2E에서 직접 검증한다.
4. 친구 카드와 친구 책나무의 **viewer에게 허용된 문장 수**가 동일하다.
5. owner·friend·nonfriend·blocked·anonymous가 hidden 문장 ID로 clap/report/count를 직접 요청해도 본문·존재·개수를 추론할 수 없다.
6. 1~1,000자는 그대로 저장하고, 1,001자 이상은 저장 시 앞 1,000 Unicode 문자로 절단했다는 비차단 경고를 표시하며 사용자가 선택한 공개범위를 유지한다.
...
- 사용자가 처음으로 `visibility=public|followers` 한 문장을 제출하기 직전에 커뮤니티 가이드라인·이용약관을 표시하고 명시적 동의를 받는다. `private` 문장은 공개 UGC가 아니므로 이 게이트를 요구하지 않는다.
