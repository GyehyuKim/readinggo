...
> **v17 목표 결정 (2026-08-19, #1452·#1454)**: 하단 목적지는 `같이읽기`이며 피드는 그 안의 보조 표면이다. 책·사용자 랭킹과 XP 보상을 제거하고, 친구 책나무와 문장 공개범위를 서버 권한대로 표시한다. 아래 §5.7.0이 목표 계약이며 기존 §5.7은 현행/이력이다.
> **편집 정책**: 이 영역 변경은 이 파일 PR로. spec-only PR 룰 준수.

### 5.7.0 v17 같이읽기 내 피드 계약
