...
- **Out**: `GyehyuKim/glocalx` 레포의 GitHub 이슈 (라벨 `source:beta-inquiry`)
...
4. **이슈 생성**: `POST /repos/GyehyuKim/glocalx/issues`
...
- `GITHUB_TOKEN`: fine-grained PAT, **단일 레포(`GyehyuKim/glocalx`)·Issues Read/Write**만. `npx wrangler secret put GITHUB_TOKEN`. 클라 노출 절대 금지(Worker 서버 전용).
