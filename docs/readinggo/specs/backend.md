...
- 책의 모든 현재·미래 문장(`sentences.text`)과 사용자가 별도로 작성한 내 생각은 부모 책의 공개 상태를 상속한다. 문장별·생각별 예외, 친구 공개, 계정 문장 기본값은 없다. 길이·입력 방식·읽기 상태·완독·재독·방 가입은 공개 상태를 바꾸지 않는다.
- **콘텐츠 유형 경계**: 공개 가능한 내 생각과 비공개 AI 대화(assistant/user 양쪽 턴)는 신규 쓰기부터 별도 필드/저장소로 분리한다. 구체적 스키마는 구현에서 정하되 `my_note` 원시값을 공개 필드로 그대로 취급하지 않는다. 사용자가 대화와 별개로 내 생각을 명시 작성하면 그 생각은 책 공개를 상속한다. 이는 문장별 privacy 토글이 아니다. 피드·이미지·story·검색·OG·재사용 metadata 등 모든 공개 출력의 생각 입력은 분리된 공개 가능 필드만 사용하며 대화 원문·대화 수·내부 session은 제외한다.
- **레거시 혼합 노트**: 생각/대화의 구분이 불명확한 `my_note`는 원문을 온전히 owner-only 저장소에 보존한다. 책 공개 확인만으로 혼합 원문을 공개하지 않으며, 소유자가 공개할 생각을 명시적으로 재작성하거나 선택해 별도 필드에 저장하기 전에는 공개 출력에서 제외한다. `Q./A.` 정규식·휴리스틱으로 추정 분리한 내용을 자동 공개하지 않는다.
...
3. 공유 직전 현재 책 접근을 다시 검사한다. 공개 전환이 성공해도 이후 clipboard/native share가 취소·실패하면 책은 공개로 남고 각각의 결과를 구분해 알린다. 비공개로 되돌리기는 책 설정에서 가능하다. 제출 전 취소 또는 서버의 미반영이 확인된 거절만 이전 상태 불변으로 안내한다. 제출 후 응답 유실·readback 실패는 결과 미확정이며 공유를 차단하고 `책 공개 설정을 확인하지 못했어요. 다시 확인해 주세요.`를 표시한다. 공유 선택·초안을 보존하고 동일 요청의 멱등 재시도 또는 현재 서버 상태 재조회로 조정한다. 조용한 rollback이나 비공개 확정 표시는 금지하며 다른 기기의 최신 변경을 오래된 재시도로 덮어쓰지 않는다.
4. private 전환 성공 후 신규 공개 요청은 문장·책·story·OG·서비스 캐시 모두 비노출이어야 한다. 권한 확인 전 캐시/snapshot을 반환하지 않고 파생 캐시를 무효화한다. 이미 외부에 저장·게시된 이미지·텍스트·제3자 미리보기 회수는 보장하지 않는다. 낙관 UI는 권위 readback의 현재 상태로 조정하며 결과 미확정이면 공개/비공개 어느 쪽도 확정하지 않는다.
...
10. DEV 검증 matrix는 owner/nonowner/anonymous/blocked/suspended × public/private/unknown × 동일 ISBN의 다른 소유자 책, 직접 UUID·count·오류·OG·story·캐시를 포함한다. 신규 기본 private, 전체 현재/미래 생각 공개, 취소/확정 거절/서버 commit 후 응답 유실/readback 실패/멱등 재시도/공유실패/중복 탭/다른 기기 철회, 혼합 노트 owner-only 보존·명시 선택 전 공개 제외·신규 대화 분리·이미지 포함 전 출력 대화 비노출, 혼합/빈 책/false note flag migration, legacy 쓰기 차단, ID·본문·timestamp 비손실을 실제 테스트한다. spec lint green은 이 구현·DB 검증을 대체하지 않는다.
...
- 정규화한 provider와 정렬된 query를 canonical cache key로 사용한다. cache hit은 외부 provider를 호출하지 않고 per-IP 분당 제한과 provider 일일 예산도 소비하지 않는다.
- cache miss만 `BookProviderGuard` Durable Object의 단일 global instance에서 provider·IP 분당 사용량과 provider 일일 사용량을 한 transaction으로 예약한다. 분당 상한은 기본 30, 일일 상한은 provider별 설정값 또는 기본 3,000이며 server-side bounded integer로 제한한다.
- 분당·일일 한도 초과는 외부 호출 전에 429로 종료한다. Production에서 Durable Object binding·예약 호출이 없거나 실패하면 503으로 fail-closed하며, 성공한 provider 200 응답만 canonical cache에 저장한다. DEV의 binding 미설정 허용은 로컬 검수용이며 Production 계약이 아니다.
- 회귀 검증은 cache hit 예산 미소비, query 순서가 달라도 같은 cache key, 동시 예약의 원자성, provider별 일일 분리, 분당·일일 429, Production binding 누락·오류 503을 포함한다.
