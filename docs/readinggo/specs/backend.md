...
### 7.0 서재·개인 활동 데이터 계약 (v18)

1. 서재의 네 상태는 기존 `user_books.status`와 `wish_books`를 권위로 사용한다. 검색·필터·정렬·carousel 때문에 별도 tree·shelf 테이블이나 전체 표지 캐시를 만들지 않는다.
2. canonical 책 검색·미등록 책 등록·중복 방지는 기존 Supabase `books`, 검색 프록시, ISBN 또는 canonical book ID 흐름을 재사용한다.
3. 로그인 시 게스트의 읽는 중·완독·중단·위시와 문장 없는 책까지 누락 없이 멱등 이관한다. 부분 실패·재시도에서 중복 행을 만들지 않고, 원격 저장 성공을 검증하기 전 로컬 원본을 지우지 않는다.
4. carousel은 클라이언트의 bounded view다. 표지 preload/decode·렌더 window 수치는 런타임 성능 계약이며 권위 데이터를 변경하지 않는다.
5. 4번째 개인 활동 방향은 우선 `reading_sessions.session_date`와 `sessions.calendar(days)`로 검토한다. 그날 읽은 책을 연결하는 데 기존 세션·책 관계가 부족하다는 증거 전에는 새 테이블을 확정하지 않는다.
6. 친구 책나무 UI와 기존 사용자 자동 활성화는 보류한다. 이미 배포된 제한 RPC/RLS는 fail-closed 상태로 보존하고 private 문장의 본문·존재·개수·오류 차이를 계속 숨긴다.
