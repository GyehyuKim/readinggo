...
// 문의 (설정 → 운영자) v7.2
inquiries.create({message})            → Inquiry             // Worker가 auth actor로 customer_feedback/admin_note 결정 (#1407)
inquiries.listMine()                   → Inquiry[]           // 본인 문의 + received/checking/answered + 답변
// 자동 답변: closed + feedback:notify-ready만 기록. webhook 대신 cron/secret-protected reconciliation.
