...
> **v18.2 활동함 결정 (2026-08-25, #1260)**: 인앱 활동함은 현재 `claps`·`follows`·`pokes`에서 90일/최대 100개를 파생하고 사용자별 `seen_through`만 영속한다. 원격 푸시·이벤트 snapshot은 추가하지 않는다. 화면 SSOT는 [activity-inbox.md](./activity-inbox.md)다.
...
activityInbox.list({limit?})           → {items, unreadCount, seenThrough, watermark} // 서버 파생 90일/최대100, activity-inbox.md
activityInbox.unreadCount()            → integer                    // 목록과 같은 moderation/현재-row 필터
activityInbox.markAllSeen(watermark)   → {seenThrough, unreadCount} // 서버 watermark 그대로, 단조 증가
// 게스트/local 어댑터는 items=[]·unreadCount=0, markAllSeen no-op. 보호 RPC나 가짜 소셜 행을 호출/생성하지 않는다.
...
activity_inbox_state                       -- #1260: 파생 활동함의 사용자별 읽음 cursor만 영속
  user_id       uuid PK FK users.id ON DELETE CASCADE
  seen_through  timestamptz NULL
