...
- **P2**: `archive` cron 인기 시드 소스 재설계(알라딘 베스트셀러 대체 — 문화 빅데이터 대출순위 등 외부 소스 + 제품 결정 필요, **#1133 Part 1 미해결**). 외서 폴백 영속 정리(`enrichForeignMeta` 의 Google merge 가 레거시 ISBN 등록 upsert 에 섞이던 잔여 경로) = **완료(#1133 Part 2)** — 영속행 Google 제외.
