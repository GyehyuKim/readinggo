...
| 4번째 개인 활동 탭 | **방향만 합의**: 월간 책 표지 달력을 1차 후보로 검토. Bookmory 화면은 활동 날짜마다 표지 한 장을 두고 같은 책을 여러 날짜에 반복함. 숫자·불꽃·최장 streak는 핵심 화면으로 채택하지 않음 | 최종 명칭, 프로필 정체성·팔로우·관리자 진입 위치, 비징벌적 보조 요약 필요성, 활동일 규칙. 하루 여러 책은 화면 근거가 없어 겹친 표지+`+N`·첫 세션 책·사용자 대표 선택을 비교. 월간 완독·재독·로컬 날짜·수정/삭제 재계산 | 4번째 탭 정식 구현 |
