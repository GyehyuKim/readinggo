...
| 기존 사용자 책 공개 (#1619, #1456 안전 후속) | **결정됨 / 구현 필요**. 책 public/private·신규 private·전체 문장/생각 상속. 기존 제한/혼합/unknown·동의 미입증은 private, 명시적 전체 scope 증거가 있으면 public 이관 | 백업·분류 근거·원본 ID/본문/timestamp 비손실·직접 역할 API 검증. 특정 소유자의 과거 문장 공개 승인에 생각까지 포함되는지는 운영자가 scope 확인 | [backend.md §7.0.1–2](../backend.md) |
| 구 문장 privacy 제거 (#1619) | **결정됨 / 구현 필요**. 새 client와 책 권한 경로 검증 뒤 구 쓰기 차단·문장 UI/기본값/setter/model/physical fields 제거. 영구 문장 예외 없음 | 지원 최소 버전·migration 번호는 구현 inventory에서 확정. broad SELECT rollback 금지 | 책 단위 공개·구 APK 컷오버 |
...
| base RLS·구 APK 컷오버 | **5-A** retained surface의 최소 view/RPC·신규 앱 선배포→최소 버전 미만 차단→broad `ub_sel` 소유자 전용 축소. 책나무 전용 API는 이관하지 않고 제거하며 보안 rollback으로 broad 정책 복원 금지 | 버전별 직접 조회 인벤토리, 스토어 바이너리 수신·호출 증거, 정확한 최소 버전, 구버전 실패·rollback 테스트 | base RLS 축소와 retained surface 안전성 |
