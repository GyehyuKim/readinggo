...
**해소 (2026-06-30, #1086)**: XP의 쓸모 = **둥지를 키우는 연료**. XP가 둥지를 키우고 → 1,600 주기마다 성이 되고 → 성·완독이 내 서재로 쌓인다(*책 행동을 더 풍부하게 하는 곳*에 쓴다 — 위 시작 질문 중 그쪽, *영속 누적되는 신분* 채택). **별도 화폐·상점 없음**(짹=화폐→상점 발상 폐기). XP·Lv 숫자 자체는 상단바에서 폐지하고 진행은 둥지·성·스트릭으로 일원화. SSOT = [systems.md §6.3 v10](../systems.md).
