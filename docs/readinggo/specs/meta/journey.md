...
자동화 점진 계획은 별도 안건으로 보존([§13](../meta/open-issues.md)).

### 0.5.5 도파민 보상의 정성

*"입력 직후 0.5초 안에 confetti + XP + 친구 박수 미리보기"* — 메카닉만 있으면 안 됨. **진짜 기분 좋게 정성껏 설계**해야 함. 현재 v4.4 세리머니는 시각적으로 충분하지만, v5에선 *친구 짹 = +1 XP* 를 추가해 **XP를 사회적 화폐로 느끼게** 한다. XP 자체의 destination·소진 메커니즘은 별도 안건([§13](../meta/open-issues.md)).

### 0.5.6 별도 안건으로 분리한 것

- **XP destination** — 모은 XP를 무엇에 쓰는지·왜 모으는지·소진되는지. 현재 v5도 누적 + 레벨업뿐. *Duolingo 벤치마킹 금지* 제약 하에 독립 발상 필요. ([§13](../meta/open-issues.md))
- **T2 mini (같은 책 자동 패널)** — [COMPETITIVE-ANALYSIS.md §7](../../COMPETITIVE-ANALYSIS.md) 옵션 C 채택 결정 완료. 세부 설계 시 **좀비 사용자 처리** 가 핵심 우려 — 같은 책으로 묶었는데 묶인 사용자들이 다 읽지 않고 있으면 새로 들어온 사용자가 *유령의 집* 을 보게 됨. ([§13](../meta/open-issues.md))
- **운영자 짹 자동화 점진 계획** — N 증가 시 어떻게 자동화로 전환할지. 학기 후. ([§13](../meta/open-issues.md))

### 0.5.7 의도적 기각 (보존)

채택 안 한 항목과 그 이유를 [§14](../meta/rejected.md)에 기록. 미래에 누가 다시 제안할 때 *이미 검토하고 기각한 이유* 를 보여주기 위함.
