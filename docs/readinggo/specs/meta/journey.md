...
- **T2 mini (같은 책 자동 패널)** — `COMPETITIVE-ANALYSIS.md [§7](../backend.md)` 옵션 C 채택 결정 완료. 세부 설계 시 **좀비 사용자 처리** 가 핵심 우려 — 같은 책으로 묶었는데 묶인 사용자들이 다 읽지 않고 있으면 새로 들어온 사용자가 *유령의 집* 을 보게 됨. ([§13](../meta/open-issues.md))
