...
## 14.3 v17 책나무 전환에서 기각한 방향 (2026-08-19)

| 항목 | 기각 사유 | 재검토 트리거 |
|---|---|---|
| XP를 새 화폐·상점·레벨로 재사용 | 제품에서 폐기하기로 한 외적 보상을 다른 표면으로 옮길 뿐이며 책·문장 성장 모델을 다시 왜곡 | 없음. 새 제품 결정 필요 |
| 사용자당 여러 나무·1,600 XP마다 새 둥지 | 사용자 한 그루·책당 가지 하나의 장기 기록성과 충돌 | 사용자 소유권 모델을 바꾸는 별도 결정 |
| 스트릭 단절·상실·복구·방패 UX | 독서 공백을 실패로 해석해 죄책감과 손실 회피를 만든다 | 없음. 리듬 데이터는 비손실 표현만 사용 |
| 미독서 자동 휴면·가지 시듦 | 사용자가 중단을 선택하지 않았는데 상태를 추정하고 부정적 시각으로 벌한다 | 명시적 사용자 선택 상태만 허용 |
| 마른/부러진 가지·낙엽 | 영속 성장·나이테 원칙과 다정한 톤에 반한다 | 없음 |
| 책·사용자·읽기방 랭킹 | 경쟁 자극이 개인 독서 기록과 같이읽기의 협력성을 침범한다 | 명시적 사용자 연구와 별도 결정 |
| 화면 전체 진화·폭죽·대형 세리머니 | 문장 저장의 빠른 흐름과 저모션 접근성을 해친다 | 없음. 짧은 인라인 피드백만 허용 |
| 별도 숲·정원 세계관 | 책나무 외에 중복 공간 은유를 만들어 IA와 용어가 흔들린다 | 없음. 그룹 기능은 `같이읽기/읽기방` |
| 사전 고지·opt-out 없는 기존 사용자 공개 확대 | 기대하지 않은 독서 상태·콘텐츠 노출과 법적·신뢰 위험. 4-B는 사전 고지 후 자동 활성화와 언제든 전체 opt-out을 필수로 하므로 이 무가드 방식과 구분한다 | 고지·opt-out·rollback 계약을 바꾸는 별도 결정 |
