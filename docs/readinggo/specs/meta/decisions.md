...
| 문장 길이·저작권 위험 | 공개범위와 무관하게 한 문장 원문은 최대 **1,000자**. OCR·직접입력·배치/import 모두 1~1,000자를 실제 저장할 수 있어야 하며, 글자 수 때문에 제외·절단하거나 `private`로 강제하지 않는다. #1424의 공개 200자 제한은 OCR 기술 한계가 아니라 공개 인용 최소화 정책 제안으로 도입됐으나 이 결정과 후속 구현 #1457로 대체한다. 공개 문장의 누적·pagination을 통한 원문 재구성 위험은 인지하되 현재 서비스가 해당 콘텐츠 자체로 부가가치를 영위하지 않는다는 제품 판단 아래 이번 v17 출시 차단 조건으로 두지 않고, 별도 법무·제품 검토로 연기한다. 출처표기·삭제요청·seed provenance 의무는 유지 |
