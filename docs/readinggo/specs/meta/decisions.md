...
| 구 APK 호환 | `users.xp`, `reading_sessions.xp_earned`, `increment_xp()`를 즉시 물리 삭제하지 않고 동결. RPC는 실제 값을 바꾸지 않고 동결값을 반환하는 no-op 호환층으로 유지 |
| 물리 삭제 게이트 | **6-A 확정**. 신규 XP 쓰기 제거→운영·분석 참조 제거→최소 지원 버전 강제→지원 버전 전체에서 legacy 호출 0 증거→release cadence에 따른 rollback 가능 기간 경과 후 RPC→세션 XP→사용자 XP 순으로 물리 삭제. 임의 날짜는 두지 않으며 UI에서만 숨긴 채 영구 유지하지 않음 |
| 성장일 | 새 권위 데이터는 `reading_sessions.session_date`의 distinct 날짜. 기본 표현은 **최근 14일 독서 리듬 + 누적 성장일** |
| 기존 스트릭 | 구 APK 호환 데이터로만 보존하고 새 UI에는 표시하지 않음. `0일`, 단절, 복구 압박, 상실 예고를 포함한 스트릭 UX는 최근 14일 리듬·누적 성장일로 대체 |
| 방패·하루 만회 | 새 UI에서 제거. 책나무 크기·가지·잎 수와 연결하지 않음 |
