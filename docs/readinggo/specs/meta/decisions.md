...
- 3번째 서재는 네 상태를 기본 포함하고 상태 chip으로 포함·제외한 단일 projection을 기존 카드 디자인의 유한한 가로 `scroll-snap` 레일로 보여준다. 상호배타적 상태 탭은 유지하지 않는다. 다음 카드 일부를 노출하고 터치·트랙패드·가로 휠·키보드를 지원한다. 무한 loop·복제 카드·넘기기만으로 활성 책 변경은 하지 않는다.
...
- 월간 활동일은 이미 사용자 로컬 `YYYY-MM-DD`인 `reading_sessions.session_date` 문자열을 그대로 사용하고, 본인 `sentences.created_at` 타임스탬프만 사용자 로컬 날짜로 변환해 합친다. 같은 날 여러 기록은 하루로 중복 제거하며 원천 수정·삭제 뒤 다시 계산한다.
