...
> **은퇴 tombstone:** 책=가지·문장=잎의 책나무 개념은 검토했으나 v18의 서재·개인 활동 구조와 충돌하고 ReadingGo의 기록·대화 가치를 흐려 제품에서 제외한다. 상세 과거 설계의 감사 기록은 Git history만 사용한다.

#### v18.1 활성 경계

| 구분 | v18 결정 |
|---|---|
| **IA** | 3번째 탭은 **서재**다. 목표 내비게이션은 `홈 / 같이읽기 / 서재 / 프로필 / 설정`이다 |
| **기록** | `user_books`·`wish_books`·`sentences`·`reading_sessions`와 실제 책 검색·등록·상세, 기록 비손실을 유지한다 |
| **책 상태** | `읽고 싶은 책`, `읽는 중`, `읽은 책`, `중단한 책` 네 상태를 하나의 서재 흐름에서 탐색한다 |
| **문장** | 공개범위와 무관하게 최대 1,000 Unicode 문자를 저장하고 사용자가 선택한 공개범위를 보존한다 |
| **개인 활동** | 사용자 로컬 독서 세션·문장 저장일을 월간 활동 캘린더와 비징벌적 연속일로 보여준다 |
| **재독** | 책당 `user_book` 하나를 유지하며 `reading_rounds`로 회차를 구분한다. 기존 `reading_round_id=NULL`은 추정 백필하지 않는다 |
| **폐기** | XP·둥지·성·방패·하루 만회와 보상·상실 압박은 복원하지 않는다. 이를 위해 구 APK 호환을 요구하지 않는다 |
| **안전** | 공개범위·moderation·privacy·fail-closed·owner-only base RLS 목표를 유지한다. 삭제 예정 전용 RPC/UI를 새 기능에서 호출하지 않는다 |
| **전달** | stable DEV에서 검증한 동일 SHA만 승인 뒤 Production으로 승격한다 |
