...
>
> 하단 탭 3번째 = **프로필**(구 "내서재"). 내 서재 + 둥지 캐릭터 + 성 컬렉션 + 설정을 담는다.
...
│ (읽은 책 탭 상단) 🏰 성 컬렉션 (3권) │   ← v8: 여기로 이동(#429)
│ [데미안] [82년생…] [불편한 편의점]    │
│ 📗 데미안 228/228p · 완독 ⭐4         │
└─────────────────────────────────────┘
```

- 상단: 닉네임·한 줄 소개(편집), 팔로워/팔로잉, **⚡ XP·Lv** 스탯 + 검색 (한 줄로 압축, 높이 절약). **v8**: 🏰성·🔥스트릭 배지는 헤더에서 제거(#425/#441) — 성 컬렉션은 §5.8.1로
- **(v7.2 QA5, #205)**: 상단과 중복되는 중간 스탯 카드(둥지레벨/완독/스트릭/XP) **제거**, 성 카드 별점 **유효숫자**(4→4.0)
- **둥지 캐릭터(NestTheatre, v8 #428)**: 헤더 바로 아래 렌더. 누적 XP 5단계([nest.md §5.2](./nest.md)) 일러스트 — LV·🔥 배지는 표시 안 함(헤더로 일원화), 진척 라벨은 누적 XP
...
- **완독은 두 형태로 남는다**: ① 🏰 **성 카운트**(성취 척도) ② **표지 이미지 책장 전시**(영구 컬렉션). 완독이 성으로만 치환되지 않는다
- **표지 책장 = 소셜 대표 화면**: 타인이 내 프로필을 열면 보는 핵심 화면 — 완독 책 표지가 컬렉션처럼 누적 전시 (전체 공개, §5.8.2)
- **선반 위치 (v8 갱신, #429)**: 서재 **"읽은 책" 탭 상단** — **완독 0권이 아닐 때만** 노출. 각 카드 = 표지 + 별점 + 완독일. 프로필 최상단 선반은 v8에서 제거(둥지 캐릭터로 대체, #428)

##### 5.8.1.1 책장 상세 (v7.4, #312)

- 서재 "읽은 책" 탭의 성 컬렉션 선반은 **최근 10권만** 미리보기.
