...
- 책 카드 탭 → 책 상세
- ⚙️ 설정 → 닉네임 변경(고유·중복검사), **내 데이터 내보내기(JSON, #172)**, **운영자 문의**(폼 → `inquiries` 저장 → admin 대시보드, [backend.md §7.2](./backend.md)), 알림 시간(Phase 2+), 로그아웃 *(한 줄 소개 편집은 #515로 프로필 헤더 인라인으로 이동 — 설정에서 제거)*
