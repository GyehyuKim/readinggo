# 같이읽기·읽기방 — 화면 스펙 `co-reading.md`

> **현행 구현 (`origin/main@39248ef`)**: P1과 일부 P2가 `함께` 탭·`숲` 화면명으로 동작한다. 아래 과거 §1~§10은 이 구현의 상세 증거다.
> **v17 목표 결정 (2026-08-19, #1410·#1452)**: 상위 기능명은 **같이읽기**, 개별 공간명은 **읽기방**이다. “둥지가 모인 숲” 결정과 별도 숲·정원 세계관은 superseded다. 아래 §0이 목표 계약으로 우선한다.
> **owner**: gyehyu. 데이터·DataStore 계약은 [backend.md §7](./backend.md), 구현 호환 식별자는 `rooms.*`·`villages`를 당장 rename하지 않는다.
> **편집 정책**: 이 영역 변경은 이 파일 PR로. spec-only PR 룰 ([LF](../../1. research_and_lectures/lecture-frameworks.md#lf-week6-spec-only-pr)) 준수.
> **Stack Lock**: 새 의존성 0 — 기존 Supabase 스키마(`villages`/`village_members`)·RLS(`is_village_member` SECURITY DEFINER)·DataStore 어댑터를 재사용한다.
...
- 그룹 목록·탐색: `읽기방 찾기`
- 개별 그룹: `읽기방`
- CTA: `읽기방 만들기`, `읽기방 참여`, `읽기방 나가기`
- 금지: `숲`, `정원`, `둥지가 모인 곳`, `숲 랭킹`, `파트 랭킹`
- 코드 식별자 `social`, `rooms`, `villages`, `.rg-room-*`, `RG_openRoom`은 migration 비용을 줄이기 위해 유지할 수 있다. UI 문자열·접근성 이름·분석 이벤트의 제품 용어만 먼저 전환한다.

### 0.2 기록 연결

- 읽기방은 책 한 권에 연결되고 참여자의 해당 `user_book`을 사용한다. 방 입장·나가기 때문에 별도 책·문장 기록을 만들지 않는다.
- 방에서 저장한 문장도 기존 `sentences` 한 행이다. 중복 복사·방 전용 문장 테이블을 만들지 않는다.
- 방 활동·방문·좋아요·마일스톤으로 XP를 적립하지 않는다.
- 재독 중인 참여자는 해당 책의 현재 `reading_round`를 사용하되, 회차 스키마 확정 전에는 방별 회차 추정을 만들지 않는다.

### 0.3 발견·공개범위·안전

- 읽기방 추천은 사용자의 읽는 책과 명시적 탐색을 우선한다. `1위`, 순위표, 리더보드, 경쟁적 파트 랭킹을 노출하지 않는다.
- 인원수는 방 수용량 판단을 위한 사실 정보로 표시할 수 있으나 우월성·보상으로 사용하지 않는다.
- 방의 문장·생각은 부모 소유자 책의 현재 공개 상태와 차단·moderation을 검증한 projection만 사용한다. 방 멤버십은 private 책 접근을 부여하지 않으며 방 가입·방 공개 전환이 책을 공개하지 않는다. 방의 공개/초대 권한과 책 privacy는 별개다.
- 신고·차단·정지·운영자 숨김은 방 멤버십보다 우선한다. 차단 상대의 콘텐츠와 프로필은 직접 API에서도 숨긴다.

### 0.4 수용기준

1. 탭·목록·상세·빈 상태·토스트·공유문에 사용자 표시 `숲`이 남지 않는다.
2. 방에서 문장 1건 저장 시 원본 문장 1행만 증가한다.
3. 방 참여·방문·좋아요로 XP가 변하지 않는다.
4. 공개범위·차단 조합의 직접 API 및 Android E2E가 통과한다.
5. 기존 방 ID·멤버십·초대 링크가 이름 변경 후에도 유지된다.

---

## 1. 현행/이전 구현 — 왜, 소셜 기둥 재설계
...
> **과거 결정 기록 (v17이 supersede)** — 당시에는 4번째 하단 탭을 추가하지 않고 기존 피드 탭을 `함께`로 재편했다. 아래 내용은 현행 구현 설명이며 신규 IA의 정본이 아니다.
...
| 요소 | 규칙 |
|---|---|
| CTA 우선순위 | **방 찾기 = Primary(솔리드)**, **만들기 = Secondary(토널/아웃라인)** — [DESIGN.md](../DESIGN.md) 버튼 위계 |
...
| 카드 요소 | 규칙 |
|---|---|
| 역사적 성장 이모지 | v18에서 폐기된 표시다. 신규 같이읽기 UI·정렬·진척 계약의 근거로 사용하지 않는다 |
...
| 카드 | **공용 `SentenceCard` + `SentenceActions`** 단일 경유([feed.md](./feed.md) §5.7 SSOT, align_v7 불변식). 타인 문장 = ❤️ 좋아요만, 내 문장 = 부모 책 설정 진입·공유·좋아요·수정·삭제(문장별 공개 토글 없음) |
...
| 기능 | 규칙 |
|---|---|
| 초대 링크 복사 | `<앱 origin>/?r=<invite_token>` (브라우저로 열면 앱이 토큰을 읽어 미리보기, #1094) |
