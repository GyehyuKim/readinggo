...
| 빈 상태 | 숲 아이콘 배지(Feather 모노라인) + "아직 들어간 숲이 없어요 · 같은 책 읽는 사람들과 같이 읽어볼까요?" + **[숲 찾기][만들기]** (#1056: 상단 액션바와 중복되던 1차 "숲 찾기 →" 버튼 제거 → 빈 카드 하나에 찾기+만들기 통합, DESIGN.md "1차 버튼 1개"). 상단 액션바(찾기·만들기)는 들어간 숲이 **있을 때만** 노출 |
