...
- 읽기방은 책 한 권에 연결되고, 참여자의 해당 책은 각자 책나무의 **같은 가지**를 사용한다. 방 입장·나가기 때문에 별도 가지나 잎을 만들지 않는다.
- 방에서 저장한 문장도 기존 `sentences` 한 행이며 해당 가지의 잎 한 장이다. 중복 복사·방 전용 문장 테이블을 만들지 않는다.
- 방 활동·방문·좋아요·마일스톤으로 XP를 적립하지 않는다.
- 재독 중인 참여자는 같은 가지의 현재 회차를 사용하되, 회차 스키마 확정 전에는 방별 회차 추정을 만들지 않는다.
...
2. 방에서 문장 1건 저장 시 원본 문장 1행과 나뭇잎 1장만 증가한다.
