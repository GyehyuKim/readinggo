...
> **v18 경계:** XP·둥지 진화·성·스트릭 상실 UX·전체 공개 피드는 활성 목표가 아니다. 책·문장 기록, 최근 14일 리듬·누적 성장일은 `specs/README.md`·`home-reading.md`·`systems.md`를 따른다.
>
...
## v7 북모리 채택 결정 (2026-06-02 · 6/7 회의 입력)
...
| 항목 | 출처(근거) | 담당 | 설명 |
|---|---|---|---|
| 무작위 한 문장 회상 | 북모리 "무작위 노트" | **계휴** | 과거 내 한 문장 1개를 프로필 카드로 다시 노출. 기억·감성 |
| 반별점 (0.5 단위) | 북모리 평점/리뷰 | **계휴** | 완독 별점([profile §5.8.3](./specs/profile.md))을 0.5 단위로. 기존 정수 별점 위 |
| 긴 메모 (my_note) | 북모리 Rich Text 노트 | **계휴** | 한 문장과 별개의 긴 감상. *너무 짧으면 안 됨* — 넉넉한 입력 |
| 한 문장 사후 감상 ★복구 | RG-원래 초안 | **계휴** | 한 문장 작성 뒤 **나중에 찾아가 그 문장에 감상을 덧붙임** (`sentences.my_note` 사후 추가·편집) |
| 완독 책 = 표지 책장 전시 ★복구 | RG-원래 초안 · 북모리 라이브러리 | **계휴** | 완독이 **🏰 성 개수로만 치환되지 않고, 표지 이미지로 책장에 영구 전시**. 타인이 프로필 누르면 보는 화면. 🏰 카운트(성취 척도)는 기존 systems 유지. [profile §5.8.1–2](./specs/profile.md) 정련 |

### B. 채택 — Phase 1 (Supabase · 외부 API)

| 항목 | 출처(근거) | 담당 | 설명 |
|---|---|---|---|
| 알라딘 OpenAPI 연동 ★허브 | 북모리 메타/바코드 · CL-09 | **계휴** | 무료 OpenAPI (TTBKey 발급). 아래 두 항목의 **공통 선행** |
| ├ 책 메타데이터 자동채움 | 북모리 | **계휴** | ISBN/제목 → 표지·저자·페이지 자동. 책 등록 마찰↓ |
| └ 컬렉션/시리즈 | 북모리 라이브러리 | **계휴** | 책 묶음 관리. *중요하나 알라딘 메타 의존 → Phase 1* |
| AI 도서 추천 (Gemini) | 북모리 큐레이션 대비 | **계휴** | [profile §5.8.6](./specs/profile.md) 이대로. 나↔책 fit |
| Supabase (Auth + DB) | RG-원래 | **계휴** | [backend.md §7](./specs/backend.md) 이대로 |

### C. 후순위 (시간 정량화 — 우리 본질 아님)

타이머 · 일일/연간 목표 · 월/연간 통계 · 독서 달력 히트맵 → **후순위**.
근거: *우리는 "시간"이 아니라 "한 문장·습관". 규칙 설정이 번거롭고 구현 대비 효용 낮음.*
**한 문장 → 이미지 카드 공유** · **다크모드/테마** 도 **후순위 개발요소** — 표현 영역(nest/design)으로 6/7 데모 범위 밖이며, 우선순위는 합의된 후속 이슈에서 결정한다. (이미지 카드: 지금도 카드 스크롤 뷰는 있음 → 단일 공유 이미지 카드가 추가분)

### D. 패스 / 기각

| 항목 | 출처 | 처리 |
|---|---|---|
| 맞춤 태그 + 취향 | 북모리 태그 | **패스** — 효용 불명확 |
| 스티커 8종 (CL-04) | 투두메이트 | **기각** — 칭찬은 좋아요(짹) 1종, 인스타식. 다종 불필요 (§6 CL-04 "채택"→번복) |
| 바코드 스캔 | 북모리 | **보류** — 네이티브(Capacitor) 의존 → Phase 3 |
| OCR · STT · 자동백업 · PIN/생체 | 북모리 | **보류** — 네이티브/서버, v7 web-first 범위 밖 |
...
> ⚠️ v5.1 기준 (Capacitor·운영자 짹·리그 등 폐기 항목 포함). **북모리 항목은 문서 상단 'v7 북모리 채택 결정' 표가 우선.**
