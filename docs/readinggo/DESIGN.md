...
> 행동·흐름(UX) 감사는 [`UX-AUDIT.md`](./UX-AUDIT.md) 체크리스트로(#1219) — 이 문서는 시각·토큰 정본.
...
- ❌ **기능 UI 이모지 금지** (📖 📚 📦 📸 📷 🔍 🔎 🔖 ✍️ ✏️ ⚙️ ✕ 🗑 🏠 ❤️ 등) → `RG_ICONS` + `rgIcon('name')` 모노라인 SVG(`icons.js`)로 통일. 컴패니언 동물 이모지는 브랜드 캐릭터 예외이며 신규 기능 아이콘의 정본이 아니다.
...
- **코어 감정:** **고양감(uplift)** — 닦달이나 상실 경고가 아니라, 나타남(showing up)과 이미 쌓인 기록을 축하하는 따뜻함
