...
> 2026-06-04 클로즈베타 감사의 역사적 기준선이다. 현재 최소권한 정본은 [`specs/backend.md §7.0.3`](./specs/backend.md)과 [`specs/feed.md §5.7.0`](./specs/feed.md)이다. 책나무 제품은 은퇴했으며 broad base-table read, 구 클라이언트 fail-open, private 문장 존재·개수·상호작용 side channel은 retained 서재·피드·프로필·활동함에서도 해결·역할별 검증 전까지 RLS 축소와 관련 Production 변경의 blocker다.
...
**2026-06 당시 감사 범위**의 위험도는 LOW–MEDIUM, Critical/High 0건이었다. 이 평가는 이후 추가된 공개범위·구 APK 전환의 안전 판정이 아니다. 현재 알려진 broad `user_books` read와 private 문장 존재 추론 side channel은 retained 서재·피드·프로필·활동함의 RLS 축소와 관련 Production 변경 전 해결해야 하는 결함이다. 은퇴한 책나무의 출시·재개 gate로 해석하지 않는다.
