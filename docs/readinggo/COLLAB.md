...
> 우선순위(모순 시): `CONTRIBUTING.md` > `CLAUDE.md` > `AGENTS.md` > `specs/README.md` > `DESIGN.md` > `ROADMAP.md` > 기타 문서(이 문서 포함).

## 0. 핵심 전제 — Slack은 GitHub를 복제하지 않는다

ReadingGo의 협업은 이미 GitHub 중심으로 규율화되어 있다: 이슈-퍼스트(§4.2), 합의된 이슈 범위의 contributor 협업, P0/P1/P2 라벨, 마일스톤 트리아지, spec-PR/code-PR 분리, 승원·윤지 self-merge 금지와 김계휴 단독 최종 merge. Slack이 메꿀 공백은 **실시간·비동기 소통 하나뿐**이다.
...
- **이슈 → 브랜치 → 드래프트 PR**: 채널에서 명시적으로 요청하면 governance(브랜치 네이밍·rebase-before-push·spec/code 분리)를 지켜 PR까지 진행한다. 봇은 merge하지 않으며, 감독 게이트를 통과한 PR의 최종 merge는 김계휴가 직접 수행한다.
