...
- 결과: 프로덕션 액세스 거절
- Google 안내: 앱을 프로덕션에 게시하기 전에 테스트가 더 필요함
- 재신청 전 요구사항: 실제 테스터와 비공개 테스트를 14일 추가 운영하고, 테스터 참여·피드백 수집·피드백에 따른 앱 업데이트를 증명
- 2026-09-06 Play Console 확인 상태: 비공개 테스트, 표시 설치 사용자 5명

기존 제출 답변의 `Testers used the core flows`, `Feedback was collected`, `We fixed key issues` 같은 문장은 실제 참여·Form 응답·issue·PR 증거와 연결되지 않는 한 재사용하지 않는다. 초대 발송 수와 신청 Form 제출 수는 Play opt-in 또는 실제 사용 증거가 아니다.

## 5. 후속 작업

- [x] 계정 소유자 이메일에서 거절 결과 확인
- [x] Play Console에서 비공개 테스트 상태 확인
- [x] 거절 사유를 신청 기록에 추가
- [x] 재테스트 운영 계약 작성
- [ ] 유효 참여자 12명 이상을 Play Console에서 확인
- [ ] D0부터 14일 테스트 운영
- [ ] 고유 피드백 12건 이상 수집·추적
- [ ] 실제 증거만으로 재신청 답변 작성

재작업 runbook: [google-play-production-access-rework-2026-09.md](./google-play-production-access-rework-2026-09.md)

## 6. 추적

- 기존 신청 이슈: #1588
- Android release 및 재신청 운영 이슈: #1583
