/* =========================================================
   ...
   같이읽기(방) P1 — co-reading.md. '함께' 탭 ② 방 레이어 + 방 내부.
   - RoomsView      : 방 목록(참여 중·추천) + 방 찾기/만들기 진입 (§4.2)
   - FindRoomSheet  : 코드·링크 / 책으로 검색 (§4.3)
   - CreateRoomSheet: 책·이름·공개설정·정원·비밀번호 (§5.1)
   - RoomPreviewSheet: 참여 전 미리보기 + 비밀번호 (§4.5)
   - RoomModal      : 방 내부 — 멤버 진척 그리드 + 한 문장 (§5.3)
   데이터는 DataStore.rooms.* (어댑터 추상화, 직접 저장소 호출 금지 — backend.md §7.2).
   명칭("방") = placeholder (co-reading.md §8, owner 확정 대기).
   ========================================================= */

/* ── 둥지 단계 이모지: 방 책 진척% → 5단계 (nest.md §5.2 SSOT 이모지) ──
   멤버 그리드는 "방 지정 책의 진척 단계"를 보여준다(개인 XP 둥지와 다른 책-진척 매핑).
...
// 방 카드 표시용 집계 — members 배열에서 인원·오늘 읽은 수·평균 진도%.
...
function FindRoomSheet({ onClose, onPreview }) {
  ...
  const submitCode = async () => {
    ...
    try {
      ...
      if (!room) { setErr('방을 찾을 수 없어요. 코드나 링크를 다시 확인해주세요.'); return; }
      onPreview(room);
    } catch (e) { setErr('방을 찾을 수 없어요. 코드나 링크를 다시 확인해주세요.'); }
  ...
  const searchBook = async (q) => {
    ...
    try {
      // 책으로 공개 방 검색 — 먼저 카탈로그에서 책 후보를 찾고 그 책들의 공개 방을 모은다.
        ...
        {tab === 'code' ? (
          ...
          <div>
            ...
            <div style={{ marginTop: 10, maxHeight: '40vh', overflowY: 'auto' }}>
              {results === null ? (
                <p className="rg-room-hint">읽고 싶은 책으로 같이 읽는 방을 찾아보세요.</p>
              ) : results.length === 0 ? (
                <p className="rg-room-hint">이 책을 같이 읽는 공개 방이 없어요. 직접 만들어볼까요?</p>
              ) : results.map(r => (
...
function CreateRoomSheet({ onClose, onCreated }) {
  ...
  const create = async () => {
    setErr('');
    if (!book || !name.trim()) { setErr('방 이름을 입력해주세요.'); return; }
    ...
    try {
      ...
      if (window.showToast) window.showToast('방을 만들었어요', { sparrow: true });
      onCreated(room);
    } catch (e) { setErr((e && e.message) || '방을 만들지 못했어요.'); }
    finally { setBusy(false); }
  };
  ...
  return (
    <div className="rg-room-sheet-backdrop" onClick={onClose}>
      <div className="rg-room-sheet" onClick={e => e.stopPropagation()}>
        <div className="rg-room-sheet-head">
          <strong>{step === 'book' ? '어떤 책을 같이 읽을까요?' : '방 만들기'}</strong>
          <button className="rg-room-x" onClick={onClose} aria-label="닫기">×</button>
        </div>
        {step === 'book' ? (
          <div>
          ...
          <div>
            ...
            <p className="rg-room-label">방 이름</p>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="방 이름" className="rg-room-input" maxLength={40} />
            ...
            <p className="rg-room-hint" style={{ marginTop: 6 }}>
              {visibility === 'public' ? '책으로 검색하면 누구나 찾을 수 있어요.' : '초대 링크·코드를 받은 사람만 들어와요.'}
            </p>
...
function RoomPreviewSheet({ room, onClose, onJoined }) {
  ...
  const join = async () => {
    ...
    try {
      ...
      if (window.showToast) window.showToast('방에 참여했어요', { sparrow: true });
      onJoined(room);
    } catch (e) {
      const m = (e && e.message) || '';
...
// 방 목록 1행 (검색 결과·추천 공용) — 표지·이름·인원·평균 + 참여 CTA
...
function RoomsView() {
  ...
  const reload = useCallback(() => {
    ...
    // 추천 = 내 활성책을 같이 읽는 공개 방 (없으면 빈 섹션). 내 책장 책별 byBook 합집합.
    Promise.resolve((DataStore.myBooks && DataStore.myBooks.list) ? DataStore.myBooks.list() : [])
      .then(async (books) => {
  ...
  return (
    <div style={{ padding: '4px 0 8px' }}>
      {/* 상단: 방 찾기(Primary) + 만들기(Secondary tonal) — DESIGN.md 버튼 위계 */}
      <div style={{ display: 'flex', gap: 8, padding: '0 16px 14px' }}>
        <button className="rg-btn-primary" style={{ flex: 1 }} onClick={() => setFindOpen(true)}>🔍 방 찾기</button>
      ...
      <div style={{ padding: '0 16px' }}>
        {mine === null ? (
          ...
          <div className="rg-room-empty">
            <p style={{ margin: 0, fontWeight: 800, color: 'var(--ink-2)' }}>아직 참여 중인 방이 없어요</p>
            <p style={{ margin: '6px 0 12px', fontSize: 13, color: 'var(--ink-3)', lineHeight: 1.6 }}>같은 책 읽는 사람들과 같이 읽어볼까요?</p>
            <button className="rg-btn-primary" style={{ width: '100%' }} onClick={() => setFindOpen(true)}>방 찾기 →</button>
          </div>
        ) : (
          <>
            <p className="rg-room-section">참여 중인 방 ({mine.length})</p>
      ...
      {recommended && recommended.length > 0 && (
        <div style={{ padding: '18px 16px 0' }}>
          <p className="rg-room-section">이 책 같이 읽는 방</p>
...
function RoomModal({ roomId, onClose }) {
  ...
  // 한 문장 탭 — 방 지정 책의 한 문장 모음 (byBook, 좋아요순). SentenceCard 경유(공용).
  useEffect(() => {
  ...
  const doLeave = async () => {
    ...
    if (window.showToast) window.showToast('방에서 나왔어요');
    setLeft(true);
    ...
    <div className="rg-room-modal-backdrop" onClick={onClose}>
      <div className="rg-room-modal" onClick={e => e.stopPropagation()}>
        <div className="rg-room-modal-head">
          ...
          <span className="rg-room-modal-title">{room ? room.name : '방'}</span>
