# CONTRIBUTING — ReadingGo 협업 규칙

이 파일은 **사람과 LLM 모두가 준수해야 하는 단일 규범**이다. Claude Code, Cursor, Copilot 등
어떤 AI 도구를 쓰든 여기 적힌 브랜치/PR/커밋 규칙을 예외 없이 따라야 한다.

> repo: `readinggo` (구 `glocalx` — 이전 프로젝트에서 피벗 후 2026-06 리네임). **프로젝트명 = ReadingGo.**

프로젝트 개요는 [`AGENTS.md`](./AGENTS.md), 스펙·팀 영역은 [`docs/readinggo/specs/README.md`](./docs/readinggo/specs/README.md) 참조.

---

## 0. 운영 권한 — 이슈는 열고, 구현·머지는 단일화

- **이슈**: 공개 저장소의 Issues는 누구나 열고, 버그·아이디어·질문·우선순위 제안을 남길 수 있다. 이슈 작성은 구현 권한이나 작업 배정을 뜻하지 않는다.
- **구현**: 코드·스펙·운영 문서 변경은 **김계휴(`gyehyu`)만** 수행한다. Hermes는 김계휴가 지시한 범위에서만 구현 작업을 보조하고 PR을 연다. 팀원의 제안은 이슈·코멘트·수용 기준으로 반영한다.
- **PR과 머지**: 모든 변경은 `main`에 직접 push하지 않고 PR로만 반영한다. 필수 CI가 모두 green이고 base가 최신인 뒤에도, Hermes가 관련 이슈·diff·변경 범위·테스트 증거·미해결 대화를 확인한 **감독 게이트**를 통과해야 한다. 그 뒤 Hermes는 김계휴를 대신해 squash merge할 수 있다.
- **중요**: CI green은 필요조건이지 자동 머지 승인이 아니다. Hermes의 확인에서 범위 이탈·회귀 위험·근거 부족이 발견되면 수정 요청 또는 보류한다.

GitHub 개인 저장소의 한계상 "이슈만 가능한 collaborator" 역할은 제공되지 않는다. 따라서 구현 권한은 김계휴만 write collaborator로 유지해 시스템적으로 제한하고, 이슈는 공개 상태로 열어 둔다.

> 이 §0은 이 문서 안의 기존 팀별 브랜치·파일 오너십 예시보다 우선한다. 기존 오너십은 제안·도메인 검토 창구로만 해석하며, 실제 변경은 `gyehyu`가 수행한다.
...
정규식: `^gyehyu/[a-z0-9]+(-[a-z0-9]+)*$`

- `owner`는 구현 권한자의 고정 slug인 `gyehyu`만 쓴다. Hermes가 만드는 브랜치도 동일하다.
...
- `seungwon/nest-castle-collection` — 팀원이 제안할 수는 있으나 구현 브랜치로 사용하지 않음
...
3. **이슈 먼저 (원칙)**: 의미 있는 작업(feat·fix·docs·perf·refactor)은 **PR을 만들기 전에** `gh issue create`로 이슈를 먼저 열고 `Closes/Refs #N`로 연결한다. `no-issue: <사유>`는 **오타·포맷·주석 같은 진짜 사소한 변경에만** 쓴다(상시 사용 금지). 헷갈리면 이슈를 먼저 만든다.
4. **⚠️ CI 강제 (`issue-link` job).** PR 본문에 `Closes/Refs #N` 또는 `no-issue: <사유>`가 없으면 **CI 빨간불 → 머지 차단**. "체크리스트라 건너뛰면 그만"이 아니라 **모든 PR(누구든)이 이슈를 연결**해야 한다 → `Closes`면 머지 시 자동 닫힘 → **이슈 정리가 한 사람에게 쏠리지 않는다.**
5. **주기적 점검(cadence)**:
...
### 4.4 UI 변경 PR — visual QA (필수)

UI(레이아웃·컴포넌트·스타일)를 바꾸는 PR은 런타임 크래시만 잡는 CI `render-smoke`로는 부족하다 — "보기에 이상한 화면"(깨진 레이아웃·모바일 오버플로·`DESIGN.md` 위반)은 사람이 봐야 한다. 아래를 직접 확인하고 PR 본문에 결과(스크린샷 권장)를 남긴다.

1. `cd docs/readinggo && npm run build && npm run smoke` (전 탭 렌더 크래시 차단)
2. **3개 뷰포트** 스크린샷 — 모바일 **390×844**, 큰 모바일 **430×932**, desktop card view
3. `DESIGN.md` 위반(이모지·raw hex·ghost·라운딩) / 깨진 레이아웃 / 모바일 오버플로 확인
4. PR 본문에 스크린샷 또는 확인 결과 첨부

> ReadingGo는 모바일 앱 감성이 핵심 → **390px에서 깨지지 않는가**를 특히 본다.
...
chore: Rename book-club.md to village.md
```
...
- **무단 구현 PR** — 팀원·외부 사용자는 이슈와 코멘트로 제안한다. `gyehyu`/Hermes 외 계정의 구현 PR·fork 기반 코드 기여는 이 운영 정책의 반영 경로가 아니다.
...
- [x] Require approvals: 0 — GitHub의 사람 리뷰 수로 자동 승인하지 않는다. 대신 §0의 Hermes 감독 게이트가 필수다.
- [x] **Require branches to be up to date before merging** ← 핵심. 웹 에디터 stale-base 사고 차단.
- [x] Automatically delete head branches (활성화 — 머지 후 자동 정리, #895)
- [x] Require status checks to pass — `render-smoke`·`boot-smoke`·`spec-align`·`contract`·`lint`·`issue-link` 필수 + strict("up-to-date 강제") (배포안전 #898)
- [x] Require conversation resolution before merging
- [x] Do not allow bypassing the above settings (관리자 포함)
- [x] force push와 branch deletion 금지
- [x] write collaborator는 `GyehyuKim`만 유지. 이슈는 public repo에서 누구나 생성 가능.

> GitHub 개인 저장소는 branch protection에서 사용자별 push restriction을 지원하지 않는다. 따라서 "단독 write collaborator + PR 보호" 조합이 이 저장소에서 구현 권한을 강제하는 실제 설정이다.
...
14. **Fork 금지.** 모든 브랜치는 `GyehyuKim/readinggo` 원본 repo에 직접 생성한다.
...
19. **구현·머지 권한(§0).** 이슈 제안자는 구현자가 아니다. 코드 변경은 `gyehyu` 또는 그가 지시한 Hermes만 수행한다. 필수 CI가 모두 green이라고 자동 머지하지 말고, Hermes가 PR의 이슈 연결·diff·테스트 근거·대화 해결을 확인한 뒤에만 머지한다.
...
- [ ] (UI 변경 시) 390 / 430 / desktop 뷰포트를 확인하고 스크린샷·결과를 첨부했는가? (§4.4)
