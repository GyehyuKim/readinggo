...
위반(emoji+hex+ghost+font)이 하나라도 있으면 exit 1. radius 는 warning 으로 카운트만 한다.
...
def main() -> int:
    ...
    blocking = len(emoji) + len(hexes) + len(ghosts) + len(fonts)
    print("\n" + "-" * 60)
    print(f"위반(exit 영향): emoji {len(emoji)} + hex {len(hexes)} + ghost {len(ghosts)} + font {len(fonts)} = {blocking}")
    print(f"warning(미반영): radius {len(radii)}")
    print("-" * 60)
    if blocking:
        print("FAIL — DESIGN.md UI 규칙 위반. 위 항목을 RG_ICONS·토큰·tonal·디자인 폰트로 교체.", file=sys.stderr)
        return 1
