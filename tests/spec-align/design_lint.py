...
탐지 카테고리
  ...
  4. radius  — 비표준 borderRadius 정수 리터럴(∉ {12,16,18}). --r-sm/md/lg 로.                 → exit 영향 O
  5. font    — 디자인 3종(Noto Sans/Serif KR·Moneygraphy R/P) 밖의 폰트 리터럴.
               표준 폴백(serif·system-ui·Georgia 등)·var(--token)·JS 변수만 허용.            → exit 영향 O

위반(emoji+hex+ghost+radius+font)이 하나라도 있으면 exit 1.
(radius 는 #1130 정합화 완료로 warning → blocking 승격.)
근거: DESIGN.md '## UI 규칙'·'## Typography', Decisions Log #1062·#1109.
"""
...
# 폰트 화이트리스트 — DESIGN.md Typography '3종 고정'(#1109) 강제.
# 모든 font-family/fontFamily 토큰은 ① 디자인 4패밀리 ② 표준 제네릭·시스템 폴백
# ③ var(--token) 중 하나여야 한다. 그 외 리터럴 폰트명 = 위반(exit 1).
# 폴백(Georgia·system-ui·serif…)은 '다른 폰트 선택'이 아니라 웹폰트 로드 전/실패 시의
# 점진적 저하라 허용 — 새 임의 웹폰트(Pretendard·Roboto 등) 유입만 차단한다.
FONT_WHITELIST = {
    # ReadingGo 디자인 폰트(DESIGN.md §Typography)
    "noto sans kr",         # Body — 본문·UI
    "noto serif kr",        # Quote — 흔적·발췌
    "moneygraphy rounded",  # Display — 히어로·큰 숫자
    "moneygraphy pixel",    # Accent — 보상 모먼트
    # 제네릭 패밀리(스택 말단 폴백)
    "serif", "sans-serif", "monospace", "system-ui",
    # OS 시스템 폰트(웹폰트 로드 전 폴백)
    "-apple-system", "blinkmacsystemfont",
    # Noto Serif KR 세리프 폴백(흔적·발췌 — DESIGN #1032)
    "georgia", "times new roman",
    # CSS 키워드
    "inherit", "initial", "unset",
}
# .html=CSS(font-family: <stack>;)  ·  .js=JSX(fontFamily: '<stack>').
# JS 값이 따옴표 없는 식(변수 _SC.fontRound 등)이면 매치 안 됨 → 정적 검사 대상 아님.
CSS_FONT_RE = re.compile(r"font-family\s*:\s*([^;{}]+)")
JS_FONT_RE = re.compile(r"fontFamily\s*:\s*(['\"])(.*?)\1")
...
def _font_token_ok(tok: str) -> bool:
    """폰트 스택 토큰 1개 허용 여부. 빈 토큰·디자인/폴백 폰트·var(--token)=OK."""
    t = tok.strip().strip("'\"").strip()
    if not t:
        return True                       # 콤마 뒤 빈 토큰
    low = t.lower()
    if low in FONT_WHITELIST:
        return True
    if low.startswith("var("):            # CSS 커스텀 프로퍼티 참조
        return True
    return False


def find_font(path: Path):
    """디자인 3종 밖의 폰트 리터럴. → [(rel, line, badtoken)]
    .html=CSS(font-family:), .js=JSX(fontFamily:'…'). JS 변수 참조(따옴표 없음)는
    리터럴이 아니라 검사 대상 아님 — 자연히 제외(예: share-card _SC.fontRound)."""
    hits = []
    if path.name in INTERNAL_SKIP_FILES:
        return hits                       # 내부 관리 툴 — 제품 UI 아님
    if path.suffix == ".html":
        code = path.read_text(encoding="utf-8")
        for ln, line in enumerate(code.split("\n"), 1):
            for m in CSS_FONT_RE.finditer(line):
                for tok in m.group(1).split(","):
                    if not _font_token_ok(tok):
                        hits.append((path.name, ln, tok.strip()))
    elif path.suffix == ".js":
        code = strip_comments(path.read_text(encoding="utf-8"))  # 주석 예시 제외
        for ln, line in enumerate(code.split("\n"), 1):
            for m in JS_FONT_RE.finditer(line):
...
def main() -> int:
    files = scan_files()
    emoji, hexes, ghosts, radii, fonts = [], [], [], [], []
    for f in files:
        emoji += find_emoji(f)
        hexes += find_hex(f)
        ghosts += find_ghost(f)
    ...
    # 4) radius
    print(f"\n[4] 비표준 라운딩 (--r-sm/md/lg 또는 pill 999):  {len(radii)} 건")
    rad_by_val = {}
    for _, _, v in radii:
    ...
    # 5) font
    print(f"\n[5] 비-디자인 폰트 리터럴 (디자인 3종+표준 폴백만):  {len(fonts)} 건")
    _examples(fonts, lambda h: f"{h[0]}:{h[1]}:{h[2]}")

    blocking = len(emoji) + len(hexes) + len(ghosts) + len(radii) + len(fonts)
    print("\n" + "-" * 60)
    print(f"위반(exit 영향): emoji {len(emoji)} + hex {len(hexes)} + ghost {len(ghosts)} + radius {len(radii)} + font {len(fonts)} = {blocking}")
    print("-" * 60)
    if blocking:
        print("FAIL — DESIGN.md UI 규칙 위반. 위 항목을 RG_ICONS·토큰·tonal·라운딩 스케일·디자인 폰트로 교체.", file=sys.stderr)
        return 1
