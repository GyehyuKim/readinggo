...
INVARIANTS = [
    ...
    ("B", "present", "큰 이미지 클라 타일링 — 세로 분할·순차 호출·병합 dedup (#1045)",
        ["shelf-import.js"], r"sliceToBlobs[\s\S]*toBlob|extractBooks[\s\S]*dedupeBooks"),
    ...
    # ── 검토함 스테이징 + 로그인 게이트 (#1048, integrated-shelf.md §4.7) ──
    ("B", "present", "shelf-import 검수 '등록' → 검토함 적재(importStaging.add, 책장 직행 아님) (#1048)",
        ["shelf-import.js"], r"importStaging\.add"),
    ("B", "present", "검토함 어댑터 — import_staging commit (supabase, addBatch 라우팅+행삭제) (#1048)",
        ["datastore-supabase.js"], r"async commit\(stagingId, status\)"),
    ("B", "present", "검토함 뷰 — 가져온 책 검토 + 내 서재로 이동(importStaging.commit) (#1048)",
        ["library.js"], r"importStaging\.commit"),
    ("B", "present", "로그인 게이트 — 게스트 shelf-import 차단(RG_openShelfImport → RG_login) (#1048)",
        ["app.js"], r"로그인하면 책을 가져올 수 있어요"),
    ...
    ("B", "present", "하단 내비 서재·프로필·설정 분리 (v18, #1518)",
        ["app.js"], r"id:\s*'library'[\s\S]*id:\s*'profile'[\s\S]*id:\s*'settings'"),
